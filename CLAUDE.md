...
- Coverage gate (crates with a real implementation must hold ≥90% line coverage; the placeholder `extension` process, the windowed reference frontend's `main.rs`, and `mcp-server`'s `main.rs`/`server.rs` -- process/transport wiring exercised by a real subprocess or MCP client, not headless-CI-reachable, with pure helpers/message-sequencing logic still unit tested (`mcp-server`'s own `lib.rs` holds the gate) -- are excluded and only reported for visibility): `cargo llvm-cov --workspace --ignore-filename-regex 'extension/src/main\.rs$|frontend-reference/src/main\.rs$|mcp-server/src/main\.rs$|mcp-server/src/server\.rs$' --fail-under-lines 90 --summary-only`
