...
- `backend/mcp-server` — `blueice-mcp-server`, an early foundation for `phase-12-mcp-server/PLAN.md`'s MCP adapter, built ahead of that phase so Chromium differential testing has a stable way to drive BlueIce now. Wraps `blueice-ipc` (`navigate`/`get_page_representation`/`click`/`type_text`/`focus`/`scroll_into_view`/`highlight`/`screenshot` MCP tools, via the `rmcp` SDK) rather than adding browsing logic of its own, per Phase 12's adapter-not-parallel-channel principle. `CoreProcess::connect` tries `blueice-launcher`'s rendezvous socket before falling back to privately spawning its own `core`, per Phase 8's minimal first slice below.
