...
**Memory-mode interaction (Phase 8):** Performance mode adds no artificial automation/DevTools cap. Recommended minimum-memory mode treats screenshots, trace buffers, DevTools/API response previews, SOAP XML trees, and `waitFor*` retention as expendable first; per-tab soft-target crossings trigger their release/streaming/early eviction but do not turn a necessary page request into an artificial hard-limit error. Extreme memory mode reserves these artifacts to the initiating tab (or the separate API-workspace account) before capture and can reject optional capture or the allocating operation once the tab/fleet hard budget is exhausted. Event streams must state an eviction/resource result rather than silently losing observable state. The shared [Phase 8 policy](../phase-8-live-core-hotswap/PLAN.md#user-selectable-memory-modes-design-resolved-not-implemented) governs the limits and reclaim order.
