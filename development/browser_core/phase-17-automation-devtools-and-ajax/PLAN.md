...
**Status**: Slice 1 ("foundations and safe inspection") is done, all four items — see "Wiring design" below for the concrete detail on each. BlueIce still has no page debugger, Selenium/WebDriver server, full Playwright-compatible client surface, Postman-like API/SOAP client, `fetch`, `XMLHttpRequest`, or script-originated request path — that is Slices 2 through 4's own scope, not started.
...
Since item 1 landed, `AutomationRequest`/`AutomationReply` also grew `OpenTab { context_id }`/`TabOpened { tab_id }` (item 2's `BrowserContextId -> TabId -> Page` layer needed a way to actually open a tab *in* a context over this protocol) and `GetAccessibilityTree`/`Screenshot` (item 4's inspection surface) — each with its own round-trip test in `backend/ipc/src/automation.rs`.

**Slice 1 item 2** (`Refactor TabManager to own the default browser context without changing existing tab_id clients`) is done: `backend/core/engine/src/tabs.rs`'s `TabManager` gained `contexts: HashMap<BrowserContextId, Vec<TabId>>` / `tab_context: HashMap<TabId, BrowserContextId>` / `next_context_id` / `default_context`, plus `default_context()`, `create_context()`, `close_context()`, `context_of()`, `context_ids()` and `open_tab_in_context()`. `TabManager::new()` now creates a `default_context` (id `1`) containing the default tab, and the pre-existing `open_tab()` is now `open_tab_in_context(self.default_context)` under the hood — every pre-Phase-17 caller (the external `ClientMessage` protocol, `blueice-mcp-server`) is byte-for-byte unaffected, exactly the plan's own requirement. 19 tests cover context creation/closing (including that closing a context closes every tab under it, without touching other contexts), ID non-reuse, and `open_tab_in_context`'s success/failure paths.

**Slice 1 item 3** (`Add an internal automation service in core, then the local-token blueice-automation adapter process. Prove two observers see the same frame/DOM generation and a controller action is reflected to both`) is done, in three layers:

- `backend/core/engine/src/automation_service.rs` — `handle_automation_request` dispatches every `AutomationRequest` variant against a live `TabManager` and an `AutomationServiceState` (per-connection granted capabilities, the shared controller lease, and the session's current frame-generation counter, mirrored in via `observe_generation`). Capability enforcement and lease exclusivity are real, not stubbed: a request without its capability gets `CapabilityNotGranted`; `Click` additionally requires a held lease; `AcquireControllerLease` fails `ControllerLeaseHeldByAnotherClient` while another connection holds it; `forget_connection` releases a departed connection's capabilities and lease. `resolve_locator` calls the real `blueice_css::select` (a new public `querySelectorAll`-shaped entry point this phase added to `blueice-css` specifically for this, with its own tests, rather than reaching into that crate's private selector-matching internals). `Evaluate`/`ApiWorkspaceSend` remain the documented `Unsupported` placeholders item 1 already described. 20 unit tests.
- Multi-connection support: `AutomationRequestSenderFactory` (`Clone`, wraps one shared `mpsc::Sender`) mints one `AutomationRequestSender` per accepted connection, all feeding the single session-owned `AutomationRequestReceiver` — proven directly by a test in which two independently-created senders enqueue onto one receiver and both get served. `session.rs`'s `run_session_with_script_and_automation_requests` drains it once per poll tick (same `POLL_INTERVAL` mechanism Phase 7's gated-navigation completions already use), after calling `AutomationServiceState::observe_generation` so `GetAccessibilityTree` can stamp its snapshot with the session's real current generation (`Page` itself has no generation counter of its own — only `session.rs`'s single session-wide one).
- `backend/automation` (crate `blueice-automation`) — the local-token adapter process. Issues a fresh 32-byte `/dev/urandom` token on every start, written to a per-user file (`default_token_path`, the same `extern "C" fn getuid()`-derived-path pattern automation.rs itself uses) with `0600` permissions; a connecting client's first frame must be `{"token": "..."}`, checked in constant time, before this process connects to `core`'s real automation socket at all and proxies bytes bidirectionally, raw and unmodified, until either side closes — a wrong token closes the connection without `core`'s socket ever being touched. Unlike the script socket, `blueice-core`'s own `--automation-socket` listener spawns one thread per accepted connection (multiple simultaneous automation observers is the whole point here). 6 unit tests in `lib.rs` (token generation/comparison/preamble-parsing, and the proxy's own logic against a fake "core" over `UnixStream::pair`) plus 6 for `main.rs`'s argument parsing, plus a 2-test real-subprocess suite (`tests/automation_binary.rs`) spawning both the real `blueice-automation` and a real `blueice-core` together.
- The two-observer proof itself: `backend/core/engine/tests/core_binary.rs`'s `real_subprocess_lets_two_automation_connections_observe_shared_tab_manager_state` spawns one real `blueice-core` with `--automation-socket`, connects two independent automation clients, has connection A (`Lifecycle`) create a context and open a tab in it, then has connection B (`Inspection`, never told that tab's ID by anything other than the shared `core` process itself) successfully `GetDom` it — and confirms B's own attempt at A's `Lifecycle`-gated `OpenTab` is rejected, proving capability grants stay genuinely per-connection even while `TabManager` state is shared. One real caveat this test (and the `blueice-automation`/TypeScript ones below) had to work around and now documents: `core`'s dispatch loop — and so all automation-request draining — doesn't start until a frontend client completes the ordinary `ClientMessage::Hello` handshake on the *primary* socket (`main` blocks in `listener.accept()` until then); an automation-only client currently cannot drive `core` before some frontend has connected at least once. That's a real, currently-unresolved limitation of this slice's process-startup order, not a documented non-goal — worth revisiting in a later slice (e.g. an automation connection triggering the same "session is live" state a frontend connection does).

**Slice 1 item 4** (`Ship DOM/AX/style/layout inspection, screenshot, navigation, locator resolution, basic waiting and input. Add the initial Elements/Accessibility DevTools panels and the TypeScript browser/context/page/locator client surface`) is done in the same deliberately-representative sense as item 1, with two commands and two client-visible tools explicitly deferred and documented rather than silently skipped:

- **Inspection**: `GetDom` (item 1), `GetAccessibilityTree` (new — the same accessibility-tree-shaped `AiSnapshot` schema `ClientMessage::GetRepresentation` already exposes externally, stamped with the session's real current generation), and `Screenshot` (new — `Page::render_visible()` → `blueice_raster::Pixmap::encode_png` → base64 on the wire). `encode_png` is a new public, in-memory (`Vec<u8>`, no temp file) PNG encoder on `Pixmap`, with `save_png` refactored to call it — `blueice-mcp-server`'s own `screenshot` tool previously round-tripped through a temp file for the same bytes this now returns directly. `AiNode`'s existing `bounds` field is this slice's layout-inspection surface; a dedicated per-node computed-style query (a DevTools "Styles pane" -- equivalent) does not exist yet and is left to a later slice.
- **Locators and waiting**: `ResolveLocator` (item 1) is the locator half. *Waiting* is explicitly not implemented in this slice: `handle_automation_request` runs synchronously on the one core session thread, so a server-side blocking wait would stall every other tab/client sharing that thread for the wait's whole duration — the same reason Phase 7's gated navigation had to become an asynchronous two-phase operation rather than a synchronous one. A caller today gets the same effect by polling `ResolveLocator` itself; a real non-blocking wait needs the same deferred-reply mechanism a later slice would also need for automation-driven navigation (see below).
- **Input**: `Click` (item 1), unchanged.
- **Navigation**: explicitly not implemented in this slice, for the same documented reason `AutomationRequest::Click` itself already gives for not following a link's href — a navigable action needs the async, gatekeeper-reviewed completion path `session.rs`'s own dispatch loop drives (`begin_gated_navigation`/`apply_completion`), which this synchronous request/reply channel doesn't reach into yet. Both this and waiting are natural candidates for Slice 2 or a dedicated follow-up once the automation dispatch boundary supports a deferred reply (the request's own `mpsc::SyncSender<AutomationReply>` reply channel would need to be handed to, and eventually fired by, a background thread the way `Completion`s already are).
- **DevTools Elements/Accessibility panels**: built as `clients/automation/`'s `devtools/inspect.ts`, a real terminal inspection tool, not a browser-hosted graphical panel — deliberately, not as a shortcut: a browser sandbox has no API to open a raw Unix-domain socket at all, so an HTML/JS panel would need its own WebSocket-bridge relay process in front of this protocol's direct-socket transport, which this slice doesn't build. The Elements panel prints the tab's raw `blueice_dom::dump` text verbatim (deliberately not re-parsed into a second tree structure in TypeScript — that would just be a second implementation of that format with its own chance to drift from the real one); the Accessibility panel is a real nested tree built from the live snapshot's own `parent`/`children` ID fields (`buildAccessibilityTree`/`formatAccessibilityTree` in `clients/automation/src/devtools.ts`, both pure and unit-tested), then printed. A real subprocess test (`clients/automation/tests/devtools_inspect_cli.test.ts`) runs the actual CLI against real spawned `blueice-core`/`blueice-automation` processes and checks its stdout. The interactive graphical panel itself (and the transport it needs) is later work.
- **The TypeScript client**: `clients/automation/` (package `@blueice/automation`) — `Browser`/`BrowserContext`/`Page`/`Locator` classes over the real wire protocol, connecting through a real `blueice-automation` adapter (never `core`'s automation socket directly, which has no authentication of its own). Runs as plain `.ts` under Node's native type-stripping — no build step, no bundler, no `tsc` dependency for this minimal slice (this repository's pinned Node version's type-stripping does reject TypeScript's constructor-parameter-property shorthand as more than type erasure, so this codebase avoids that one syntax form). `Browser.page(tabId)` is a small necessary addition beyond a literal reading of "browser/context/page/locator": this protocol has no tab-enumeration command of its own (unlike the external `ClientMessage::ListTabs`), so attaching to an already-known tab ID (starting with the default tab every fresh `core` process has) needs some entry point other than `BrowserContext.newPage`. 11 tests total: 6 pure unit tests of `devtools.ts`'s tree-building/formatting, and 5 real-subprocess tests (`client.test.ts`, `devtools_inspect_cli.test.ts`) spawning real `blueice-core`/`blueice-automation` binaries plus small Node-native stand-ins for a clearing `ai-gatekeeper` and a fixture HTTP server (neither part of the client library itself, just test setup for getting real content into a tab this synchronous protocol can't navigate to on its own yet).
...
- [x] Refactor `TabManager` around browser contexts while retaining the current default-tab wire compatibility — Slice 1 item 2, see "Wiring design" above
- [x] Build the local, authenticated automation adapter and inspection/locator/input slice — Slice 1 items 3-4, see "Wiring design" above
- [ ] Build the first-party DevTools Elements, Accessibility, Console, Network and Sources panels — Slice 1's own initial Elements/Accessibility panels are done as a terminal tool (`clients/automation/devtools/inspect.ts`), see "Wiring design" above; an interactive graphical panel, and Console/Network/Sources, remain later work
...
- [x] Publish and test the `@blueice/automation` browser/context/page/locator client library — `clients/automation/`, see "Wiring design" above (a representative Slice 1 subset, not the full eventual Playwright-shaped surface Slice 3 still asks for)
