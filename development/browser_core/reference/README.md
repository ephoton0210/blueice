...
Shallow, read-only clones of the two upstream projects BlueIce reads directly as technical reference and a porting basis (plan §4). These are third-party source, not BlueIce's own code — `gecko/` and `chromium/` are gitignored and never committed; only specific files actually ported out of them (with the header conventions from [`CONTRIBUTING.md`](../../../CONTRIBUTING.md)) become part of this repo.
