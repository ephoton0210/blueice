...
This raised an engineering question, now resolved for the reference implementation: `frontend` needs full-frame pixel output every frame, a very different traffic shape from `extension`/AI's occasional semantic calls, so it gets a separate high-bandwidth channel (real `mmap`-backed shared memory, one file per frame generation) alongside the same control-plane IPC protocol, rather than serializing whole frames through it — see `research/frontend-ipc.md` and [Phase 4](phase-4-human-rendering-path/PLAN.md). `extension`/AI actually sharing this same control-plane protocol (rather than each having its own) is still open.
...
**Fleet-wide memory, not just `core`'s own budget**: `core`'s low-memory requirement doesn't automatically extend to the whole multi-process fleet above — [`research/multi-process-memory.md`](research/multi-process-memory.md) found neither Chromium nor Firefox has a true whole-fleet memory budget either, only per-process limits, so this needed independent design rather than copying a pattern. Recommendation, applied per-process: `core` and `ai-gatekeeper` stay always-resident (the latter because Phase 7's fail-closed policy treats its unavailability as risk, not because it's cheap); `mcp-server` is on-demand-spawned (stateless adapter, no reason to run with no MCP client connected); `downloads` and `ai-assistant` idle-teardown when inactive (with a live-translation carve-out for the latter); `extension` instances get a bounded pool with idle-teardown rather than unbounded growth. The Phase 8 launcher, already supervising `core`'s own lifecycle, is the proposed owner of this fleet-wide idle-teardown authority rather than a separate coordinator role.
...
| [Phase 15](phase-15-chromium-differential-testing/PLAN.md) | Chromium differential testing: BlueIce's rendering checked against a real Chromium via Puppeteer | In progress (full local pipeline works end to end — `npm run differential`; found and fixed a real `blueice-html` text-node-merging bug on its first run, now 28/28 fixtures DOM-identical, 99.8% average pixel match on the one remaining known MVP-scope visual gap; CI wiring and cross-run trend tracking still open) |

Phases 7-12 extend the browser beyond the rendering-engine MVP (Phases 0-6) into a full application platform — added once that broader scope was set, not part of the original MVP path. Several (7, 8 especially) have open design questions blocking real work; see each phase's own doc. Phase 13 (BlueJS) is different from 7-12 in that respect — it resolves Phase 2's JS-strategy decision and is genuinely MVP-relevant (Phase 2's HTML/CSS scope already assumes `<script>` exists), numbered last only because inserting it earlier would have meant renumbering everything else. Phases 14 and 15 are similar in kind to Phase 13: both raised mid-stream rather than planned from the start (i18n while retrofitting the credits screen; Chromium differential testing once Phase 5 made BlueIce drivable enough to need it), both numbered last for the same renumbering-avoidance reason — see each one's own plan for what's actually done versus what's still open.
