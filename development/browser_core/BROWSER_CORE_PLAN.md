...
| [Phase 15](phase-15-chromium-differential-testing/PLAN.md) | Chromium differential testing: BlueIce's rendering checked against a real Chromium via Puppeteer | In progress (full local pipeline works end to end — `npm run differential` — with a real first result: 27/28 fixtures DOM-identical, 99.8% average pixel match; CI wiring and cross-run trend tracking still open) |
