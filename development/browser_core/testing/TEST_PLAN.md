...
- **Unit tests**: per-crate, colocated `#[cfg(test)] mod tests` (standard Rust convention), testing one function/type's behavior in isolation. `blueice-dom`, `blueice-html` (`backend/core/html/src/{tokenizer,tree_builder}.rs`), `blueice-css` (`backend/core/css/src/{tokenizer,selector,value,parser,cascade}.rs`), and `blueice-layout` (`backend/core/layout/src/{text,block}.rs`) are the current reference examples.
- **Integration/fixture tests**: cross-crate, exercising the real pipeline end to end (e.g. Phase 3's planned "fixture HTML+CSS → asserted paint output" smoke test). These belong in each crate's `tests/` directory once there's a real pipeline to assert against. `blueice-html` has two: a hand-written one (`backend/core/html/tests/parsing.rs`, against the public `parse` API) and the shared-corpus one (`backend/core/html/tests/fixtures.rs`, built on `blueice-testing` — see "Rendering-correctness fixtures" below). `blueice-css` and `blueice-layout` each have the equivalent shared-corpus test (`tests/fixtures.rs` in each crate) — `paint` is still a stub, so the full-pipeline version of this waits on it.
- **Rendering-correctness fixtures** (settled design; DOM-shape, computed-style, and layout-geometry slices implemented): a single corpus of small HTML(+CSS) fixtures under `development/browser_core/testing/fixtures/*.dat`, closer to how WPT/reftests (and html5lib-tests specifically) work than to unit tests. Each fixture is a `#data` (HTML input) plus one named section per pipeline stage that can check something about it — `#document` (a canonical DOM-tree dump), `#css`+`#styles` (an optional author stylesheet, plus a canonical computed-style dump checked with the UA stylesheet always applied), and `#layout` (a canonical fragment-geometry dump — see "UI testing strategy" below) exist today; `paint` adds `#paint`, Phase 5 adds `#ai-representation`, all without changing this format, any earlier stage's section, or any already-written fixture file — this is the concrete answer to needing one stable cross-stage interface instead of one rebuilt per phase. `layout.dat`'s fixtures are new files; `basic.dat`'s first fixture also gained a `#layout` section directly (on top of the `#styles` section it already gained from the CSS stage), proving a fixture written for one stage keeps picking up later stages' checks with zero changes to the sections or tests that already existed. The shared crate `blueice-testing` (`backend/testing/`) owns only the generic format parser and the DOM dump serializer (`dump_dom`); each pipeline crate's own `tests/` directory (`backend/core/{html,css,layout}/tests/fixtures.rs`) owns its own section's dump/compare logic and runs every fixture through *that crate's real public API* per the Definition of Done above, so adding a new fixture file (or a new section to an existing one) needs zero new code in `blueice-testing` or in another stage's tests. The DOM dump format is adapted directly from the html5lib-tests tree-construction corpus (`| `-prefixed, 2-space-per-depth, sorted attributes as their own lines, text as `"content"`) rather than invented, and a handful of fixtures are that corpus's own verbatim cases (from the local Gecko/Chromium checkouts under `reference/`) used as an independent, browser-vendor-verified oracle — not just self-authored expectations. This is also how two real bugs were caught: exact whole-tree-shape comparison surfaced runaway nesting in the adoption agency algorithm (new formatting element inserted on the wrong side of `furthest_block` in the stack, `blueice-html`) that coarser tag/text-only assertions had missed entirely, and building a genuine multi-section fixture (CSS content containing an ID selector like `#x { ... }`) surfaced a bug in the fixture-format parser itself (any bare `#`-prefixed line was assumed to be a new section marker) that a single-section corpus could never have exercised.
- **UI tests**: split into two genuinely different things — see "UI testing strategy" below. The fixture-based geometry/visual layer (`#layout`, soon `#paint`) is actionable *now* and already exists; real platform window/interaction automation is not yet actionable (there is no frontend; Phase 4 hasn't started) — treat any claim of the latter before Phase 4 as wrong.
...
- **Crates with real implementation** must hold **≥90% line coverage**, enforced in CI via `--fail-under-lines 90`. Currently `blueice-dom` (98%+), `blueice-html` (90%+ tokenizer/tree-builder, 100% on the crate's public entry point), `blueice-css` (93%+ across tokenizer/selector/value/parser/cascade), `blueice-layout` (95%+ across fragment/text/block), and `blueice-testing` (98%+) — the shared fixture harness gets held to the same bar as product code, since a bug in the thing everything else is verified against is worse than a bug in any one crate.
- **Stub crates** (`paint`, `engine`, `extension`, `ipc` — anything whose body is `todo!()`, or that can't yet be exercised because something it calls still is) are excluded from the enforced number via `--ignore-filename-regex`, since a coverage percentage on unreachable stub code is meaningless. They're still measured and reported every run for visibility, so nobody can quietly land real logic into a "stub" crate without a test gap showing up. `blueice-engine`'s own wiring code is real (not `todo!()`), but it calls into `blueice-paint`, which still is, so it stays excluded until paint lands too.
- **When a stub crate gets its first real implementation, removing it from the ignore-filename-regex list is part of that crate's own Definition of Done** — tracked as a checklist item in whichever phase implements it (currently Phase 3 for `paint`/`engine`; `html`, `css`, and `layout` already did this).
...
cargo llvm-cov --workspace \
  --ignore-filename-regex '(core/(engine|paint)/src/lib\.rs|extension/src/main\.rs)$' \
  --fail-under-lines 90 \
  --summary-only
```
...
**1. Rendered-output verification — gated on the pipeline, not on a frontend. Started now, in Phase 3.** This is "what would appear on screen," checked automatically against fixtures, with no window, no frontend, and no human looking at anything:
...
- `#paint` (next, once `blueice-paint` is real): a canonical paint-command or raster dump, the same way — extends the exact same fixture files and format, per the "one stable interface, extended not refactored" design.
...
None of this needs Phase 4. It's the concrete, already-integrated answer to wanting automated UI/visual verification without waiting for a real window to look at.

**2. Real frontend/platform automation — gated on Phase 4's frontend existing. Not yet actionable.** Once a frontend exists, two layers, only the first of which is meaningfully automatable across all platforms:
...
Neither of *this* section's layers exists yet, and can't before Phase 4's frontend does. Updating this half of the section with what's actually built is part of Phase 4's Definition of Done, not a separate later task.
