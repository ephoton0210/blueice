...
- **Integration/fixture tests**: cross-crate, exercising the real pipeline end to end (e.g. Phase 3's planned "fixture HTML+CSS → asserted paint output" smoke test). These belong in each crate's `tests/` directory once there's a real pipeline to assert against. `blueice-html` has two: a hand-written one (`backend/core/html/tests/parsing.rs`, against the public `parse` API) and the shared-corpus one (`backend/core/html/tests/fixtures.rs`, built on `blueice-testing` — see "Rendering-correctness fixtures" below) — `css`/`layout`/`paint` are still stubs, so the full-pipeline version of this waits on them.
- **Rendering-correctness fixtures** (settled design, DOM-shape slice implemented): a single corpus of small HTML fixtures under `development/browser_core/testing/fixtures/*.dat`, closer to how WPT/reftests (and html5lib-tests specifically) work than to unit tests. Each fixture is a `#data` (HTML input) plus one named section per pipeline stage that can check something about it — only `#document` (a canonical DOM-tree dump, see below) exists today; `css` will add `#styles`, `layout` adds `#layout`, `paint` adds `#paint`, Phase 5 adds `#ai-representation`, all without changing this format, any earlier stage's section, or any already-written fixture file — this is the concrete answer to needing one stable cross-stage interface instead of one rebuilt per phase. The shared crate `blueice-testing` (`backend/testing/`) owns the format parser and the DOM dump serializer (`dump_dom`); each pipeline crate's own `tests/` directory (e.g. `backend/core/html/tests/fixtures.rs`) depends on it and runs every fixture through *that crate's real public API* per the Definition of Done above, so adding a new fixture file needs zero new Rust code. The dump format is adapted directly from the html5lib-tests tree-construction corpus (`| `-prefixed, 2-space-per-depth, sorted attributes as their own lines, text as `"content"`) rather than invented, and a handful of fixtures are that corpus's own verbatim cases (from the local Gecko/Chromium checkouts under `reference/`) used as an independent, browser-vendor-verified oracle — not just self-authored expectations. This is also how a real bug was caught: exact whole-tree-shape comparison surfaced runaway nesting in the adoption agency algorithm (new formatting element inserted on the wrong side of `furthest_block` in the stack) that coarser tag/text-only assertions had missed entirely; see the fix's comment in `blueice-html`'s `adoption_agency`.
...
- **Crates with real implementation** must hold **≥90% line coverage**, enforced in CI via `--fail-under-lines 90`. Currently `blueice-dom` (98%+), `blueice-html` (90%+ tokenizer/tree-builder, 100% on the crate's public entry point), and `blueice-testing` (98%+) — the shared fixture harness gets held to the same bar as product code, since a bug in the thing everything else is verified against is worse than a bug in any one crate.
