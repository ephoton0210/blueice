...
| BlueTS / BlueTSC / direct bridge | `bluets/src/checker.rs` (1,466), `checker/tests.rs` (1,361), `compiler.rs` (1,310), `bluets-bluejs/src/lib.rs` (1,260), `parser/declarations.rs` (1,215) | Project-wide type surfaces, ambient host declarations, per-module binding/expression checks, declaration parsing, type/cursor parsing, runtime-token boundaries, direct-expression lowering, page-realm ownership, and regression suites have focused children. Every BlueTS, BlueTSC, and `bluets-bluejs` production/test source is at or below the requested 1,500-line boundary; the largest is `checker.rs` at 1,466. |
...
| BlueTS checker/parser/direct lowerer/page bridge | `checker/{project,module/}`, `parser/{declarations,type_syntax,runtime_syntax}`, and `bluets-bluejs/{expression,page_runtime,debug_attachment,tests/}` | `checker.rs`, 1,466 |
