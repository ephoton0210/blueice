...
**Memory-policy follow-on (Phase 8):** this stable `TabId` is also the sole key for `TabMemoryAccount`; do not infer ownership from a page pointer, IPC connection, or process because all tabs share `core` and may share BlueJS. Performance mode attaches no artificial account limit. Recommended minimum-memory mode supplies a soft target that triggers tab-local minimization but not a forced error. Extreme memory mode adds the per-tab hard limit, which must be checked before allocations are committed; its fleet cap and OS enforcement are launcher-owned. See [Phase 8](../phase-8-live-core-hotswap/PLAN.md#user-selectable-memory-modes-design-resolved-not-implemented).
