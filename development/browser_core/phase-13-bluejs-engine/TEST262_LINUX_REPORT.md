...
Timeout-remediation working-tree run. Adapter SHA-256: `3e7f32efcbe10c8f0d0d8bd2a3dc3b9aebce5f899e885d7d00d38ca2f6c17883`. Platform: Ubuntu 24.04.4 LTS x86_64. Test262 revision: `72faf8ec1445c55149615e8b35187830783aba1a`; unfiltered `main` including proposals and staging; six jobs selected by the portable `min(8, logical CPUs)` default.
...
The run completed all 53,582 test files in 601.166 seconds. Its JSONL contains zero `timeout` and zero `harness_error` records. The 21,120 retained failures are semantic outcomes, not filtered or recategorized timeouts.
...
platform parity until it has. This snapshot predates this same branch's
later Duration/Instant/PlainTime/Now arithmetic work (see
[Phase 26's plan](../phase-26-ecma262-temporal/PLAN.md) for current status);
it is retained here as historical measurement evidence, not current status.
