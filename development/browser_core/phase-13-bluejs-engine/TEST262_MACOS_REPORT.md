...
Each measured cell shows covered / instrumented followed by the coverage rate. All 171 Rust files under `backend/bluejs/src/` are listed: 158 have counters in the LLVM export. The other 13 use `-` for coverage that does not apply to the file; the `Note` column explains each case. Seven contain only module declarations or re-exports, and six contain test code rather than coverage targets. These 13 files do not contribute to the total denominator. A `0%` result is reserved for a file with instrumented code and zero covered units; no measured file has that result here. The total aggregates the 158 instrumented files. Region coverage measures LLVM source regions and is separate from branch coverage.

| Source file (relative to `backend/bluejs/src/`) | Lines | Functions | Regions | Note |
| --- | ---: | ---: | ---: | --- |
