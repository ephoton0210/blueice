...
The [2026-09-10 continuation report](INTL_CONFORMANCE.md) supersedes the fixed-default locale behavior and absent regex timeout described in earlier slices above. String locale methods now use ICU locale/case/collation data and Intl.Collator/getCanonicalLocales; Intl.Locale provides canonical locale objects, Unicode options, likely-subtag transforms and Locale-info queries; Math constants/functions and regex compilation/matching run in the same bounded VM and terminable subprocess respectively. Declarative and assignment array/object patterns, object spread, global numeric conversion functions and JSON's data-only `parse`/`stringify` paths are also present. The 2026-09-13 Rust 1.95 no-exclusion coverage measurement was 30,342/34,330 lines (88.38%); the [current BlueJS coverage record](../../../README.md#current-bluejs-coverage-2026-09-24) supersedes it and records line, function and region rates. The CI line floor is 88%, and the earlier 100% claim applies only to a much smaller implementation slice. The Node oracle count is historical. The 2026-09-22 [Ubuntu Test262 report](TEST262_LINUX_REPORT.md) recorded **102,859 / 102,926** passing modes at commit `9decbb3`, including **6,714 / 6,714** ECMA-402 modes. The later [macOS full-inventory analysis](TEST262_ANALYSIS_REPORT.md) records **102,921 pass, 0 fail, 1 stale corpus, 4 host-capability exclusions** at commit `fbcbe2c2`; this is not a full-conformance or cross-platform claim.
