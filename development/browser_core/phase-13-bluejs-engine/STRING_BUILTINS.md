...
The [2026-09-09 continuation report](INTL_CONFORMANCE.md) supersedes the fixed-default locale behavior and absent regex timeout described in earlier slices above. String locale methods now use ICU locale/case/collation data and Intl.Collator/getCanonicalLocales; Intl.Locale provides canonical locale objects, Unicode options, likely-subtag transforms and Locale-info queries; regex compilation and matching run in a terminable subprocess. Current validation is 239 default tests plus two docs, 7,445/7,445 covered lines and 22,226 Node oracle scripts. The full pinned Test262 inventory has 9,776 passes, 56,234 failures and 36,568 unsupported modes; remaining Intl constructors and supporting language/global/host semantics prevent a complete conformance claim.
