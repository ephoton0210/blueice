...
The [2026-09-09 continuation report](INTL_CONFORMANCE.md) supersedes the fixed-default locale behavior and absent regex timeout described in earlier slices above. String locale methods now use ICU locale/case/collation data and Intl.Collator/getCanonicalLocales; Intl.Locale provides canonical locale objects, Unicode options, likely-subtag transforms and Locale-info queries; Math constants/functions and regex compilation/matching run in the same bounded VM and terminable subprocess respectively. Current validation is 246 default tests plus two docs, a passing no-exclusion BlueJS coverage gate and 22,243 Node oracle scripts. The full pinned Test262 inventory has 12,481 passes, 62,911 failures, 26,284 unsupported modes and 902 resource timeouts; remaining Intl constructors and supporting language/global/host semantics prevent a complete conformance claim.
