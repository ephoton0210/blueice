...
**Status**: In progress. `backend/bluets` provides a standalone, host-neutral BlueTS front end and `bluetsc` command for an explicitly bounded initial language matrix. `backend/bluets-bluejs` now also proves a direct, host-neutral classic-script path: it checks caller-supplied BlueTS, lowers a deliberately small runtime subset to the public BlueJS program AST, and compiles it to BlueJS bytecode without reparsing BlueTSC's emitted JavaScript. It does **not** yet execute a TypeScript page: the page-script host, module bridge, bytecode safe-point map, debugger IPC and MCP project-registration boundary remain prerequisites.
...
3. Extend the shipped `BlueJsProgramV1` classic-script hand-off to an origin-preserving page/module AST/IR, with compiler/cache resource accounting and safe-point validation.
...
- [x] Implement the first public BlueJS structured-program hand-off for a bounded host-neutral classic-script subset, without emitted-source reparsing
