...
**Status**: In progress. `backend/bluets` provides a standalone, host-neutral BlueTS front end and `bluetsc` command for an explicitly bounded initial language matrix. `backend/bluets-bluejs` now also proves direct, host-neutral classic-script and single-module ESM paths: it checks caller-supplied BlueTS, lowers a deliberately small runtime subset to the public BlueJS program AST, and compiles it to BlueJS bytecode without reparsing BlueTSC's emitted JavaScript. It does **not** yet execute a TypeScript page: the page-script host, resolved multi-module graph, bytecode safe-point map, debugger IPC and MCP project-registration boundary remain prerequisites.
...
- `blueice-bluejs` owns the public `BlueJsProgramV1` wrapper and its `bluejs-program-v1` identity. `backend/bluets-bluejs` is the sole crate that depends on both compilers. It consumes one checked, already-tokenized BlueTS source module and directly constructs the BlueJS AST before BlueJS compiles bytecode; it never gives BlueTSC-emitted JavaScript to the BlueJS parser. Its executable v1 subset is one non-declaration classic script or one local-export ESM module with erasable types, `var`/`let`/`const` declarations, named local functions with required identifier parameters, ordered local-declaration/`return` bodies, and literal/identifier/parenthesized arithmetic/direct-call expressions. The ESM mode maps local named and default exports to BlueJS module entries. The parser records opaque function-body tokens so the bridge rejects rather than silently drops any statement it cannot lower. Imports, re-exports, member access, optional/default/rest parameters and other runtime shapes fail explicitly until their direct lowering is implemented.
...
3. Extend the shipped single-source `BlueJsProgramV1` script/module hand-off to an origin-preserving resolved page/module graph AST/IR, with compiler/cache resource accounting and safe-point validation.
...
- [x] Implement the first public BlueJS structured-program hand-off for bounded host-neutral classic-script and local-export ESM-module subsets, without emitted-source reparsing
