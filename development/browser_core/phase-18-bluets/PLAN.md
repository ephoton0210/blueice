...
- [x] Implement VM-independent `BlueTsDebugInfo` (source hashes, symbols, static types and spans); direct and isolated child page admission retain it only against an exact live BlueJS generation, with child metadata never crossing page-host IPC. Bytecode source mapping and controlled debugger exposure remain pending
