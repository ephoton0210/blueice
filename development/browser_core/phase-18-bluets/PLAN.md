...
**Status**: In progress. `backend/bluets` provides a standalone, host-neutral BlueTS front end and `bluetsc` command for an explicitly bounded initial language matrix. `backend/bluets-bluejs` now also proves direct, host-neutral classic-script and resolver-preserving ESM-module-graph paths: it checks caller-supplied BlueTS, lowers a deliberately small runtime subset to the public BlueJS program AST, and compiles it to BlueJS bytecode without reparsing BlueTSC's emitted JavaScript. `blueice_engine::compiler_service` provides a core-owned in-memory registered-project check/build boundary with fixed closed inputs and generation-bound static metadata. `blueice_ipc::compiler` plus `blueice_engine::compiler_ipc` define its bounded query-only adapter for owner-minted project/generation handles (`describe`, `check`, and individual static type/symbol lookup); no IPC request carries a path, source graph, resolver, plugin, compiler option, output path, write elevation, or source-read capability. A core startup owner can now register closed projects into `CoreCompilerProjectCatalog`, seal it before the session starts, and opt into `blueice-core --compiler-socket`; the listener handshakes independently and forwards only opaque queries to the core session. The reference binary exposes only a compiled-in closed fixture profile for real-process coverage, not a project-path/source CLI. `blueice-mcp-server` can be explicitly connected to that separately negotiated service and exposes source-free `bluetsc_check`, `debug_get_type`, and `debug_get_symbol` tools; its normal browser-only startup leaves those tools unavailable rather than creating a project or falling back to local compilation. Launcher-owned catalog distribution, project updates, artifact/output transaction capability, and complete negotiated MCP surface are still absent. `blueice-core` can run discovered inline BlueTS declarations only when started with a core-selected `--inline-bluets-profile`, and can report bounded source-free outcomes over frontend IPC; that controlled seam is not the production page-script host. It can also opt into a separate `--inline-bluejs` standard-JavaScript host: classic and ESM declarations run in a bounded HTTP(S)-origin tab/document realm with immutable `blueiceDocumentText()` and canonical `blueiceDocumentOrigin()` snapshots but no DOM object or event binding, external sources fail closed unless a core owner supplies a closed canonical graph, and the frontend can drain source-free JavaScript outcomes. The flags are mutually exclusive because the current experimental owners would otherwise create two realms for one page. Its separate opt-in debugger socket now provides generation-checked source-free program/safe-point discovery and lifecycle-bound breakpoint configuration; pause/resume, stepping, stack, scope, and value inspection remain planned. The launcher-managed/out-of-process host, production source policy/cache, shared JavaScript/BlueTS realm, bytecode safe-point map aggregation, native debugger operations, and negotiated MCP boundary remain prerequisites.
