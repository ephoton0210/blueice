...
The current direct bridge subset supersedes earlier phase summaries: literals/identifiers, bounded quoted-string and template escapes, `${identifier}` template slots, direct calls, supported unary/binary/logical/conditional/assignment/sequence expressions, arrays, identifier/string/numeric object keys with identifier shorthand, dot/bracket reads, property assignments, and identifier-only updates. General template expressions, optional chaining, member calls, computed object keys, spread, methods, and accessors remain outside the subset.
