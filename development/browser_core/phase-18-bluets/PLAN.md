...
- [x] Implement VM-independent `BlueTsDebugInfo` (source hashes, symbols, static types and spans); direct and isolated child page admission retain it only against an exact live BlueJS generation. Page-host v8 can enumerate only a separately minted source-free opaque metadata handle for an exact live BlueTS program and describe it with a bounded fingerprint/count summary. Debugger v7 defaults to a denied negotiated manifest; an owner must separately enable `OpaqueInventory` and dependent `OpaqueSummary`, and a client must request both for the exact live realm. The public summary is handle/generation-bound and carries only language/options fingerprints plus source/type/symbol/contract counts—not source identity/text, spans, names, type/symbol/contract records, bytecode, or values. Static record disclosure, bytecode source mapping, and controlled debugger read exposure remain pending.
