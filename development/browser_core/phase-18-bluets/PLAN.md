...
- [x] Implement VM-independent `BlueTsDebugInfo` (source hashes, symbols, static types and spans); direct and isolated child page admission retain it only against an exact live BlueJS generation. Page-host v7 can enumerate only a separately minted source-free opaque metadata handle for an exact live BlueTS program; the static metadata itself, child program IDs, and all public debugger exposure remain pending. Bytecode source mapping and controlled debugger exposure remain pending
