...
- [x] Implement VM-independent `BlueTsDebugInfo` (source hashes, symbols, static types and spans); direct and isolated child page admission retain it only against an exact live BlueJS generation. Page-host v7 can enumerate only a separately minted source-free opaque metadata handle for an exact live BlueTS program. Debugger v6 defaults to a denied negotiated manifest; only an owner-enabled, client-requested, exact-live-realm `OpaqueInventory` operation remints at most one public handle, with no metadata read or source/type/symbol/span/contract/bytecode/value payload. Static metadata disclosure, bytecode source mapping, and controlled debugger read exposure remain pending.
