...
Named local function bodies may use a direct-expression `if` condition with braced consequent and optional braced `else` bodies. BlueTSC retains each body structurally and the bridge emits BlueJS `Stmt::If` with explicit blocks, including nested braced `if` statements. The checker applies its existing bounded direct-expression checks to the condition and recursively to the branch bodies; it does not claim control-flow narrowing. Unbraced branches, `else if`, loops, and every other control-flow form remain opaque and fail closed at the direct bridge.
