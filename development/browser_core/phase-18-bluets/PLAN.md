...
Named local functions may use a final identifier rest parameter. The direct bridge uses BlueJS's rest-parameter AST form; with an explicit `T[]` annotation, BlueTSC checks every normal or tuple-spread tail argument against `T` and infers generic `T` from the same tail. Optional/default parameters and general array/iterable spread analysis remain outside the direct rule.
