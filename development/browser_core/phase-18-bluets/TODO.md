...
**Current leaf: C3.1.3.4.7.5.4.3.2.** Extract the child socket connection
transport and its `PageHostClient` implementation into a bounded internal
module, preserving the public connection type and protocol behavior.
...
- [ ] **C3.1** Map nested/module locations, breakpoints, symbols, and type
  ...
  - [ ] **C3.1.3** Show static types separately from runtime values in every relevant debugger reply.
    ...
    - [ ] **C3.1.3.4** Add an independently granted public static scope relation request/reply with exact ordinary Scopes or linked-stack-derived scope receipt plus metadata/type receipts; retain the existing separate bounded `Value` reply, bump public protocol only when the complete route and IPC/core denials pass.
      ...
      - [ ] **C3.1.3.4.7** Prove ordinary classic/module/nested and linked real Launcher sockets, same-stream receipts, no-partial denials, and separate `Value` authority; then audit BlueTS code modularity before checking off C3.1.3.4.
        ...
        - [ ] **C3.1.3.4.7.5** Audit BlueTS source and test file responsibilities after C3.1.3.4.7.4. Assess each file over 1,300 lines for cohesive module boundaries, refactor oversized files where a maintainable split is justified, preserve coverage and public behavior, then close C3.1.3.4.7 and C3.1.3.4.
          ...
          - [ ] **C3.1.3.4.7.5.4** Modularize the BlueTS-owned islands in oversized runtime, debugger, protocol, compiler-service, and public-socket test files; preserve exact grants, receipts, and wire formats. Refine each file-scoped leaf if one concern still exceeds the reviewable size.
            ...
            - [ ] **C3.1.3.4.7.5.4.3** Extract BlueTS-owned child-executor adapter logic from `javascript_child.rs` into internal modules. Preserve the public child-client/connection API, exact debugger grants, and child wire replies; subdivide the large core debugger adapter by concern before closing the parent.
              ...
              - [ ] **C3.1.3.4.7.5.4.3.2** Extract `PageHostConnection` socket transport and its `PageHostClient` implementation into a bounded module without altering deadlines or reply validation.
            ...
            - [ ] **C3.1.3.4.7.5.4.6** Split BlueTS debugger and child protocol shape/validator owners in IPC, preserving exact serialization.
            - [ ] **C3.1.3.4.7.5.4.7** Split BlueTS compiler IPC, service, and MCP owners by transport/policy/query responsibility without changing public routes.
            - [ ] **C3.1.3.4.7.5.4.8** Split oversized Launcher and core-binary real-process acceptance tests by BlueTS concern, sharing one fixture harness and preserving coverage.
