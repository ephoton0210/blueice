...
Named local function bodies may use a direct-expression `if` condition with braced consequent and optional braced `else` bodies. BlueTSC retains the condition and each branch as structured body items; the bridge lowers them to BlueJS `Stmt::If` with explicit blocks, including nested braced `if` statements. The checker recursively applies its existing bounded direct-expression checks without claiming control-flow narrowing. Unbraced branches, `else if`, loops, and every other control-flow form remain opaque and rejected by this v1 bridge.
