...
Named local functions may declare a final identifier rest parameter. The direct bridge lowers it to the BlueJS rest-parameter AST shape; BlueTSC's bounded call rule uses an explicit `T[]` annotation to check every supplied tail argument and infer a generic `T`. Optional and default parameters remain excluded from direct lowering.
