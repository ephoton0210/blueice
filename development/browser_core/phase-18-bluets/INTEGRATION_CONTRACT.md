...
Array literals accept normal and spread elements, and preserve non-spread holes as BlueJS AST holes rather than materializing `undefined` properties. A literal that combines any hole with a spread element MUST be rejected: BlueJS's current spread-construction path would materialize the hole. BlueTSC infers a known `T[]` spread source as element type `T`; a non-array or unknown spread source remains `unknown` in this bounded rule.
