...
The current wrapper does not assign stable AST node IDs or source spans. BlueJS now provides the limited `bluejs-program-debug-v1` registry foundation: a caller-authorized host can install a compiled structured program with an immutable canonical module/source-hash identity, receive a monotonically fresh program generation, enumerate root-first code-unit IDs and instruction boundaries, and validate a `(code_unit, offset)` tuple against that exact live generation. The host-neutral bridge can hand off its already compiled direct script/module bytecode and the exact BlueTS-retained source identity to that registry without a generated-JavaScript parse or a second compilation pass. Replacing or invalidating a program removes its old generation, so stale IDs fail closed rather than being reinterpreted for successor bytecode. This foundation does not yet provide AST node provenance, a page lifetime, a pause hook, stack/scope inspection, or a TypeScript-to-safe-point map. Those remain required before direct-page source/debug attachment. The following logical envelope is therefore the target direct-page extension, not a representation currently transferred by the host-neutral script/module bridge:
