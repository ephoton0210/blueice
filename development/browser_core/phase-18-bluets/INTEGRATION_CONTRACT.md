...
Named local functions may declare a final identifier rest parameter. The direct bridge lowers it to the BlueJS rest-parameter AST shape; BlueTSC's bounded call rule uses an explicit `T[]` annotation to check every supplied tail argument and infer a generic `T`. Optional parameters without defaults remain excluded from direct lowering.

Named local functions may also declare default parameters whose initializer is in the direct expression subset. BlueTSC retains the original initializer tokens, checks a typed initializer against its parameter annotation, and BlueTS-to-BlueJS lowers it to BlueJS `Param::default`; an optional parameter without an initializer remains excluded.
