...
**Status**: In progress — the implementation is modular by Temporal value type and operation. The authoritative Ubuntu 2026-09-19 full Test262 inventory passes 12,682 / 13,268 combined `built-ins/Temporal/` and `intl402/Temporal/` modes (95.583%), with 586 remaining failures and no Temporal timeouts. Earlier per-merge figures below are historical; Stage 3 tracks the remaining compatibility, coverage and cross-platform work.
