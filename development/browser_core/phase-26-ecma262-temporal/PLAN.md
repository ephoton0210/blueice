...
summed. **Stage 2's `PlainDate`/`PlainDateTime` slice is done** (single
owner, sequential, per this document's own Stage 2 design — see that
section's own bullet for the full account): `PlainDate` 384→1,886/2,290
(82.4%), `PlainDateTime` 382→2,056/2,512 (81.8%), and the whole-tree
`Temporal/` number to **7,576/13,272 (57.1%)**, with zero regressions
anywhere else in `Temporal/` on the same full-tree run.
`plain_year_month.rs`/`plain_month_day.rs` and `zoned_date_time.rs` remain
open, in that order, per Stage 2's own stated sequencing. It exists because
