...
question). Stage 1 (parallel tracks) is under way: Track C
(`Temporal.Instant` arithmetic) and Track B (`Temporal.Duration` arithmetic)
have landed — see their own entries for measured Test262 numbers and for
exactly which calendar-dependent cases each deferred to Stage 2 — and
Tracks A, D and E have not started. It exists because
