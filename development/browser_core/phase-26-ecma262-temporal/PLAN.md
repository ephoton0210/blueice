...
The available local environment is **Ubuntu 24.04.3 LTS under WSL2** (`x86_64-unknown-linux-gnu`, Rust/Cargo 1.95.0), not Ubuntu 24.04.4. The Temporal/Test262 figures recorded throughout this long-lived plan are dated per-merge snapshots. On 2026-09-19, this environment completed the unfiltered Test262 inventory: 53,582 files / 102,926 modes in 685.257 seconds with eight workers. The current full-run Temporal result is **12,682 / 13,268 (95.583%)** with no Temporal timeouts; this section is authoritative over older per-merge snapshots. It is a real Ubuntu 24.04.3 result, not a 24.04.4 result.

macOS and Windows runs are deferred until those platforms are available. Their future values must be obtained from real runs, never copied from Ubuntu. The exact Ubuntu scope, including the isolated-target regex-worker caveat, is in [the Ubuntu Test262 report](../phase-13-bluejs-engine/TEST262_LINUX_REPORT.md).

**Temporal-only run on macOS (2026-09-20, `feature/temporal-stage-3` at `ba5ef3d`).** A real, filtered (not full-inventory) run on Darwin 25.6.0 / arm64 with Homebrew cargo 1.98.0 (no `rustup`, so `rust-toolchain.toml`'s 1.95.0 pin was **not** in effect), eight workers, pinned corpus `72faf8ec`, `--filter built-ins/Temporal/,intl402/Temporal/,staging/Temporal`: **13,268 / 13,272 modes pass** in 69 seconds, with no timeouts. By tree: `built-ins/Temporal/` 9,210 modes and `intl402/Temporal/` 4,058 modes (13,266 / 13,268 passing between them, **99.985%**, the two failures both in `intl402/Temporal/ZonedDateTime/prototype/toLocaleString/offset-time-zones.js`), and `staging/Temporal/` 4 modes (2 passing). Those four failures were then fixed (see "Closure round 2" in Stage 3), after which the same selection plus all of `intl402/` and `built-ins/Date/` passes **17,164 / 17,164**. A further 79 files (158 modes) that carry `features: [Temporal]` but live outside those directories (65 in `intl402/DateTimeFormat/prototype/`, 8 in `built-ins/Date/prototype/`, 6 in `intl402/DurationFormat/prototype/`) were run separately and all pass. This is a macOS result, not a Windows one, and it does not replace the Ubuntu full-inventory figure.

**Status**: In progress — the implementation is modular by Temporal value type and operation. The authoritative Ubuntu 2026-09-19 full Test262 inventory passes 12,682 / 13,268 combined `built-ins/Temporal/` and `intl402/Temporal/` modes (95.583%), with 586 remaining failures and no Temporal timeouts. Stage 3 closure rounds 1 and 2 (both 2026-09-20) have since taken the Temporal-only figure to 13,272 / 13,272 on macOS (all of `built-ins/Temporal/`, `intl402/Temporal/` and `staging/Temporal/` passing; non-Temporal `intl402/` 2,656 / 2,656 as well), which is a Test262 result, not a claim of complete conformance: the remaining work is coverage, a Windows run and an Ubuntu full-inventory re-run. Earlier per-merge figures below are historical; Stage 3 tracks the remaining compatibility, coverage and cross-platform work.
