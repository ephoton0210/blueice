...
impl Vm {
    ...
    fn temporal_calendar_fields(
        ...
        // ICU4X gives the ISO calendar a synthetic single `"default"` era
        // purely for its own internal era/extended-year bookkeeping
        // (`icu_calendar::cal::iso::IsoEra`) — Temporal's own `iso8601`
        // calendar has no era at all, so `era`/`eraYear` must observe
        // `undefined` there (Test262's `TemporalHelpers.assertPlainDate`
        // calls this on every fixture, since it defaults `era`/`eraYear` to
        // `undefined`). Every other calendar keeps ICU4X's own era, if any.
        let era = if value.calendar == "iso8601" {
            None
        } else {
            year.era()
        };
