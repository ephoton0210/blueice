...
impl Vm {
    pub(super) fn intl_global(&mut self) -> Result<Value, RuntimeError> {
        ...
        let result = (|| {
            ...
            self.install_native(namespace, function_prototype, "Locale", 1, NativeFunction::Locale)?;
            let constructor = self.heap.get(namespace, "Locale")?.object_id().unwrap();
            let prototype = self.with_roots(|heap| heap.alloc_object(Some(object_prototype)))?;
            self.define_data(constructor, "prototype", Value::Object(prototype), false, false, false)?;
            self.define_data(prototype, "constructor", Value::Object(constructor), true, false, true)?;
            self.define_data(prototype, JsSymbol::well_known("toStringTag"), Value::String("Intl.Locale".into()), false, false, true)?;
            self.install_native(prototype, function_prototype, "toString", 0, NativeFunction::LocaleToString)?;
            self.install_native(prototype, function_prototype, "maximize", 0, NativeFunction::LocaleMaximize)?;
            self.install_native(prototype, function_prototype, "minimize", 0, NativeFunction::LocaleMinimize)?;
            for (name, operation) in [
                ...
                self.install_native(prototype, function_prototype, name, 0, NativeFunction::LocaleInfo(operation))?;
            }
            for (name, getter) in [
                ...
                self.install_getter(prototype, function_prototype, name.into(), &format!("get {name}"), NativeFunction::LocaleGetter(getter))?;
            }
    ...
    pub(super) fn canonical_locales(&mut self, locales: &Value) -> Result<Vec<Locale>, RuntimeError> {
        ...
        for index in 0..length {
            ...
            let locale = if let Value::Object(id) = value {
                if let Some(locale) = self.heap.intl_locale(id)? { locale.locale.clone() } else { intl::canonicalize(&self.coerce_string(&Value::Object(id))?)? }
            } else {
    ...
    fn locale_unicode_option(&mut self, options: &Value, name: &str) -> Result<Option<UnicodeValue>, RuntimeError> {
        let Some(value) = self.string_option(options, name, &[])? else { return Ok(None) };
        if value.is_empty() || !value.split('-').all(|part| (3..=8).contains(&part.len()) && part.bytes().all(|byte| byte.is_ascii_alphanumeric())) {
            return Err(RuntimeError::RangeError(format!("invalid {name} option")));
        }
        Ok(Some(UnicodeValue::try_from_str(&value).unwrap()))
    }

    fn locale_set_keyword(&mut self, locale: &mut Locale, options: &Value, name: &str, key: &str) -> Result<(), RuntimeError> {
        if let Some(value) = self.locale_unicode_option(options, name)? {
            locale.extensions.unicode.keywords.set(key.parse().unwrap(), value);
        }
        Ok(())
    }

    fn locale_first_day_of_week(&mut self, options: &Value) -> Result<Option<UnicodeValue>, RuntimeError> {
        let Some(value) = self.string_option(options, "firstDayOfWeek", &[])? else { return Ok(None) };
        ...
        if value.is_empty() || !value.split('-').all(|part| (3..=8).contains(&part.len()) && part.bytes().all(|byte| byte.is_ascii_alphanumeric())) {
            return Err(RuntimeError::RangeError("invalid firstDayOfWeek option".into()));
        }
        Ok(Some(UnicodeValue::try_from_str(value).unwrap()))
    }

    fn locale_instance(&mut self, locale: Locale, prototype: ObjectId) -> Result<Value, RuntimeError> {
        self.with_roots(|heap| heap.alloc_intl_locale(Rc::new(intl::Locale { locale }), prototype)).map(Value::Object)
    }

    pub(super) fn create_locale(&mut self, args: &[Value], construct: bool) -> Result<Value, RuntimeError> {
        if !construct {
            return Err(RuntimeError::TypeError("Intl.Locale must be called with new".into()));
        }
        self.intl_global()?;
        ...
        let mut locale = match tag {
            ...
            _ => return Err(RuntimeError::TypeError("locale tag must be a String or Object".into())),
        };
        let options = self.intl_options(native::argument(args, 1))?;

        if let Some(language) = self.string_option(&options, "language", &[])? {
            locale.id.language = language.parse::<Language>().map_err(|_| RuntimeError::RangeError("invalid language option".into()))?;
        }
        if let Some(script) = self.string_option(&options, "script", &[])? {
            locale.id.script = Some(script.parse::<Script>().map_err(|_| RuntimeError::RangeError("invalid script option".into()))?);
        }
        if let Some(region) = self.string_option(&options, "region", &[])? {
            locale.id.region = Some(region.parse::<Region>().map_err(|_| RuntimeError::RangeError("invalid region option".into()))?);
        }
        if let Some(variants) = self.string_option(&options, "variants", &[])? {
            ...
            let mut parsed =
                variants.split('-').map(|value| value.parse::<Variant>().map_err(|_| RuntimeError::RangeError("invalid variants option".into()))).collect::<Result<Vec<_>, _>>()?;
            parsed.sort();
        ...
        if let Some(hour_cycle) = self.string_option(&options, "hourCycle", &["h11", "h12", "h23", "h24"])? {
            locale.extensions.unicode.keywords.set("hc".parse().unwrap(), UnicodeValue::try_from_str(&hour_cycle).unwrap());
        }
        if let Some(case_first) = self.string_option(&options, "caseFirst", &["upper", "lower", "false"])? {
            locale.extensions.unicode.keywords.set("kf".parse().unwrap(), UnicodeValue::try_from_str(&case_first).unwrap());
        }
        let numeric = self.get_property(&options, &"numeric".into())?;
        if numeric != Value::Undefined {
            let value = if primitive::truthy(&numeric) { UnicodeValue::default() } else { UnicodeValue::try_from_str("false").unwrap() };
            locale.extensions.unicode.keywords.set("kn".parse().unwrap(), value);
        }
        self.locale_set_keyword(&mut locale, &options, "numberingSystem", "nu")?;
        if let Some(value) = self.locale_first_day_of_week(&options)? {
            locale.extensions.unicode.keywords.set("fw".parse().unwrap(), value);
        }
    ...
    pub(super) fn locale_data(&self, receiver: &Value) -> Result<Rc<intl::Locale>, RuntimeError> {
        let id = receiver.object_id().ok_or(RuntimeError::TypeError("receiver is not an Intl.Locale".into()))?;
        self.heap.intl_locale(id)?.ok_or(RuntimeError::TypeError("receiver is not an Intl.Locale".into()))
    }

    pub(super) fn locale_to_string(&self, receiver: &Value) -> Result<Value, RuntimeError> {
        Ok(Value::String(self.locale_data(receiver)?.locale.to_string().into()))
    }

    pub(super) fn locale_transform(&mut self, receiver: &Value, maximize: bool) -> Result<Value, RuntimeError> {
        ...
        self.intl_global()?;
        let constructor = self.globals["%Intl.Locale%"];
        let prototype = self.heap.get(constructor, "prototype")?.object_id().unwrap();
        self.locale_instance(locale, prototype)
    }

    pub(super) fn locale_getter(&self, receiver: &Value, name: native::LocaleGetter) -> Result<Value, RuntimeError> {
        ...
        let value = match name {
            ...
            native::LocaleGetter::Script => locale.id.script.map(|value| Value::String(value.to_string().into())).unwrap_or(Value::Undefined),
            native::LocaleGetter::Region => locale.id.region.map(|value| Value::String(value.to_string().into())).unwrap_or(Value::Undefined),
            native::LocaleGetter::Variants => {
                let variants = locale.id.variants.to_string();
                if variants.is_empty() { Value::Undefined } else { Value::String(variants.into()) }
            }
            native::LocaleGetter::Calendar => intl::keyword(locale, "ca").map(|value| Value::String(value.into())).unwrap_or(Value::Undefined),
            native::LocaleGetter::Collation => intl::keyword(locale, "co").map(|value| Value::String(value.into())).unwrap_or(Value::Undefined),
            native::LocaleGetter::HourCycle => intl::keyword(locale, "hc").map(|value| Value::String(value.into())).unwrap_or(Value::Undefined),
            native::LocaleGetter::CaseFirst => intl::keyword(locale, "kf").map(|value| Value::String(value.into())).unwrap_or(Value::Undefined),
            native::LocaleGetter::Numeric => Value::Bool(intl::keyword(locale, "kn").is_some_and(|value| value != "false")),
            native::LocaleGetter::NumberingSystem => intl::keyword(locale, "nu").map(|value| Value::String(value.into())).unwrap_or(Value::Undefined),
            native::LocaleGetter::FirstDayOfWeek => intl::keyword(locale, "fw").map(|value| Value::String(value.into())).unwrap_or(Value::Undefined),
        };
        Ok(value)
    }

    fn locale_info_array(&mut self, values: Vec<String>) -> Result<Value, RuntimeError> {
        self.array_from(values.into_iter().map(|value| Value::String(value.into())).collect())
    }

    pub(super) fn locale_info(&mut self, receiver: &Value, operation: native::LocaleInfo) -> Result<Value, RuntimeError> {
        ...
        match operation {
            ...
            native::LocaleInfo::Collations => {
                let collation = intl::keyword(&locale, "co").filter(|value| value != "standard" && value != "search").unwrap_or_else(|| "emoji".into());
                self.locale_info_array(vec![collation])
            }
            native::LocaleInfo::HourCycles => {
                let hour_cycle = intl::keyword(&locale, "hc").unwrap_or_else(|| if locale.id.language.as_str() == "en" { "h12".into() } else { "h23".into() });
                self.locale_info_array(vec![hour_cycle])
            }
            native::LocaleInfo::NumberingSystems => {
                let numbering_system = intl::keyword(&locale, "nu").unwrap_or_else(|| if locale.id.language.as_str() == "ar" { "arab".into() } else { "latn".into() });
            ...
            native::LocaleInfo::TextInfo => {
                ...
                let rtl = script.as_deref().is_some_and(|script| matches!(script, "Arab" | "Hebr" | "Syrc" | "Thaa" | "Nkoo" | "Adlm" | "Rohg"))
                    || matches!(locale.id.language.as_str(), "ar" | "arc" | "ckb" | "dv" | "fa" | "he" | "ks" | "ku" | "nqo" | "ps" | "sd" | "syr" | "ug" | "ur" | "yi");
                let prototype = self.object_prototype;
                let result = self.with_roots(|heap| heap.alloc_object(Some(prototype)))?;
                self.define_data(result, "direction", Value::String(if rtl { "rtl" } else { "ltr" }.into()), true, true, true)?;
                Ok(Value::Object(result))
            }
            native::LocaleInfo::TimeZones => {
                let Some(region) = locale.id.region else { return Ok(Value::Undefined) };
            ...
            native::LocaleInfo::WeekInfo => {
                let first_day = match intl::keyword(&locale, "fw").as_deref() {
                    Some("mon") => 1.0,
                    Some("tue") => 2.0,
                    Some("wed") => 3.0,
                    Some("thu") => 4.0,
                    ...
                    Some("sat") => 6.0,
                    Some("sun") => 7.0,
                    _ if locale.id.region.as_ref().is_some_and(|region| matches!(region.as_str(), "US" | "CA" | "JP")) => 7.0,
                    _ => 1.0,
                };
                let prototype = self.object_prototype;
                let result = self.with_roots(|heap| heap.alloc_object(Some(prototype)))?;
                self.stack.push(Value::Object(result));
                self.define_data(result, "firstDay", Value::Number(first_day), true, true, true)?;
