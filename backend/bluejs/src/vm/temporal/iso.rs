...
pub(crate) fn parse_duration_record(source: &str) -> Option<blueice_ecma402::DurationRecord> {
    ...
    // A fractional component may only appear on the *last* unit present, so
    // it terminates the duration (Temporal's `DurationTime` grammar).
    ...
    while characters.peek().is_some() {
        ...
        if characters.peek() == Some(&'T') {
            ...
            characters.next();
            in_time = true;
            // `T` must introduce at least one time component.
            characters.peek()?;
            continue;
        }
        ...
        if let Some(fraction) = fraction {
            // Only the three time units carry a fraction, and it cascades
            // exactly into the units below it: `PT1.03125H` is one hour plus
            // 1m52.5s, not a rounded hour count.
            ...
            // Exact, with no rounding: the scale is a power of ten at most
            // 10^9, and 10^9 divides each of the three unit lengths above.
            let scale = 10_i128.pow(u32::try_from(fraction.len()).ok()?);
            let mut remainder = fraction.parse::<i128>().ok()? * (unit_nanoseconds / scale);
            for (slot, unit) in [
