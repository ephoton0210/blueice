...
/// `DifferenceTemporalPlainYearMonth`'s numeric core (steps 5-12), with
/// `date1`/`date2` the ISO dates of the two months' *first days* (what
/// `CalendarDateFromFields` yields for `{ year, month, day: 1 }`), and `largest`
/// and `smallest` restricted to `month`/`year` by the caller. It is the same
/// midnight-to-midnight difference as [`difference_plain_date`], with two
/// differences:
///
/// - Equal year-months are zero *before* their first days are judged, so
///   `minYearMonth.since(minYearMonth)` is exact although `-271821-04-01` is
///   not a valid date; any other pair needs both first days representable
///   (`since/throws-if-year-outside-valid-iso-range.js`).
/// - Rounding is skipped when `smallest` is `month` with an increment of 1
///   (step 11), not `day`.
///
/// `None` when a first day or a rounding bracket leaves the representable
/// range; the caller maps that to the specification's `RangeError`.
pub(crate) fn difference_plain_year_month(
    calendar: AnyCalendarKind,
    date1: CivilDate,
    date2: CivilDate,
    largest: TemporalUnit,
    ...
    if !epoch::is_date_within_limits(date1) || !epoch::is_date_within_limits(date2) {
        return None;
    }
    difference_between_midnights(
        calendar,
...
mod tests {
    ...
    #[test]
    fn a_year_month_difference_skips_rounding_only_for_a_single_month() {
        let (year, month) = (TemporalUnit::Year, TemporalUnit::Month);
        // 2019-01 .. 2021-09 is 2 years 8 months, or 32 months.
        let (early, late) = ((2019, 1, 1), (2021, 9, 1));
        assert_eq!(
            year_month_diff(early, late, year, 1, month, Mode::Trunc),
            Some([2, 8, 0, 0, 0, 0, 0, 0, 0, 0])
        );
        assert_eq!(
            year_month_diff(early, late, month, 1, month, Mode::Trunc),
            Some([0, 32, 0, 0, 0, 0, 0, 0, 0, 0])
        );
        assert_eq!(
            year_month_diff(late, early, year, 1, month, Mode::Trunc),
            Some([-2, -8, 0, 0, 0, 0, 0, 0, 0, 0])
        );
        // An increment, or a coarser smallest unit, does round: 8 months is
        // 3/5 of the way from 5 to 10 (trunc keeps 5), and 2y8m is 2/3 of the
        // way to 4 years (halfExpand reaches it).
        assert_eq!(
            year_month_diff(early, late, year, 5, month, Mode::Trunc),
            Some([2, 5, 0, 0, 0, 0, 0, 0, 0, 0])
        );
        assert_eq!(
            year_month_diff(early, late, year, 4, year, Mode::HalfExpand),
            Some([4, 0, 0, 0, 0, 0, 0, 0, 0, 0])
        );
        assert_eq!(
            year_month_diff(early, late, month, 10, month, Mode::Trunc),
            Some([0, 30, 0, 0, 0, 0, 0, 0, 0, 0])
        );
    }

    #[test]
    fn equal_year_months_are_zero_before_their_first_day_is_judged() {
        // -271821-04-01 is not a representable date, yet the same year-month
        // twice is exactly zero.
        let (year, month) = (TemporalUnit::Year, TemporalUnit::Month);
        assert_eq!(
            year_month_diff(
                (-271821, 4, 1),
    ...
    #[test]
    fn a_year_month_first_day_outside_the_representable_range_is_reported() {
        let (year, month) = (TemporalUnit::Year, TemporalUnit::Month);
        let epoch = (1970, 1, 1);
        for outside in [(-271821, 4, 1), (275760, 9, 14)] {
            assert_eq!(
                year_month_diff(outside, epoch, year, 1, month, Mode::Trunc),
                None,
                "{outside:?} first"
            );
            assert_eq!(
                year_month_diff(epoch, outside, year, 1, month, Mode::Trunc),
                None,
                "{outside:?} second"
            );
        }
        // The first day of the earliest *usable* month, and of the latest, are fine.
        assert!(year_month_diff((-271821, 5, 1), epoch, year, 1, month, Mode::Trunc).is_some());
        assert!(year_month_diff((275760, 9, 1), epoch, year, 1, month, Mode::Trunc).is_some());
    }

    #[test]
    fn a_year_month_rounding_bracket_beyond_the_representable_range_is_reported() {
        let (year, month) = (TemporalUnit::Year, TemporalUnit::Month);
        assert_eq!(
            year_month_diff(
                (1970, 1, 1),
