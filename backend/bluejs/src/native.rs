...
pub(crate) const STRING_METHODS: &[(&str, u32, StringMethod)] = &[
    ...
    ("blink", 0, StringMethod::Html { tag: "blink", attribute: "" }),
    ("bold", 0, StringMethod::Html { tag: "b", attribute: "" }),
    ("fixed", 0, StringMethod::Html { tag: "tt", attribute: "" }),
    ("fontcolor", 1, StringMethod::Html { tag: "font", attribute: "color" }),
    ("fontsize", 1, StringMethod::Html { tag: "font", attribute: "size" }),
    ("italics", 0, StringMethod::Html { tag: "i", attribute: "" }),
    ("link", 1, StringMethod::Html { tag: "a", attribute: "href" }),
    ("small", 0, StringMethod::Html { tag: "small", attribute: "" }),
    ("strike", 0, StringMethod::Html { tag: "strike", attribute: "" }),
    ("sub", 0, StringMethod::Html { tag: "sub", attribute: "" }),
    ("sup", 0, StringMethod::Html { tag: "sup", attribute: "" }),
];
...
pub(crate) fn uint32(value: &Value) -> Result<u32, RuntimeError> {
    let number = primitive::number(value)?;
    Ok(if number.is_finite() { number.trunc().rem_euclid(4294967296.0) as u32 } else { 0 })
}
...
pub(crate) fn append(result: &mut JsString, part: &JsString, limit: usize) -> Result<(), RuntimeError> {
    if result.byte_len().checked_add(part.byte_len()).is_none_or(|bytes| bytes > limit) {
...
pub(crate) fn from_codes(args: &[Value], points: bool, limit: usize) -> Result<Value, RuntimeError> {
...
pub(crate) fn substitution(string: &JsString, matched: &JsString, position: usize, template: &JsString, limit: usize) -> Result<JsString, RuntimeError> {
    ...
    while index < units.len() {
        ...
        append(&mut result, &JsString::from_code_units(part.to_vec()), limit)?;
        index += if expansion.is_some() { 2 } else { 1 };
    }
    Ok(result)
}

pub(crate) fn string_method(method: StringMethod, receiver: &Value, args: &[Value], limit: usize) -> Result<Value, RuntimeError> {
    use StringMethod::*;
    ...
    if matches!(method, ToString | ValueOf) && !matches!(receiver, Value::String(_)) {
        return Err(RuntimeError::TypeError("String value method requires a String receiver".into()));
    }
    ...
    Ok(match method {
        At | CharAt | CharCodeAt | CodePointAt => {
            ...
            if index < 0.0 || index >= len as f64 {
                ...
                if method == CharCodeAt || method == CodePointAt {
                    let point = if method == CodePointAt && (0xd800..=0xdbff).contains(&unit) && units.get(index + 1).is_some_and(|unit| (0xdc00..=0xdfff).contains(unit)) {
                        (u32::from(unit) - 0xd800) * 0x400 + u32::from(units[index + 1]) - 0xdc00 + 0x10000
                    } else {
                        u32::from(unit)
                    };
        ...
        Slice | Substring => {
            ...
            let end = if matches!(second, Value::Undefined) { len as f64 } else { integer(second)? };
            let bound = |index: f64| {
                let index = if method == Slice && index < 0.0 { len as f64 + index } else { index };
                index.clamp(0.0, len as f64) as usize
            };
            let (start, end) = (bound(start), bound(end));
            let (start, end) = if method == Substring { (start.min(end), start.max(end)) } else { (start, end.max(start)) };
            ...
            match method {
                ...
                _ => {
                    let index = if needle.len() > len {
                        ...
                        (0..=position.min(len - needle.len())).rev().find(|&index| units[index..].starts_with(needle))
                    } else {
        ...
        IsWellFormed => Value::Bool(char::decode_utf16(units.iter().copied()).all(|unit| unit.is_ok())),
        ToWellFormed => {
            let mut result = JsString::default();
        ...
        PadStart | PadEnd => {
            ...
            let padding: Vec<_> = fill.as_code_units().iter().copied().cycle().take(target as usize - len).collect();
        ...
        Repeat => {
            ...
            if count < 0.0 || count == f64::INFINITY {
                return Err(RuntimeError::RangeError("invalid String repeat count".into()));
            }
        ...
        Trim | TrimStart | TrimEnd => {
            let space = |unit: &u16| char::from_u32(u32::from(*unit)).is_some_and(primitive::whitespace);
            let start = if method == TrimEnd { 0 } else { units.iter().take_while(|unit| space(unit)).count() };
            ...
            let form = match method {
                ...
                _ => {
                    ...
                    if !["NFC", "NFD", "NFKC", "NFKD"].iter().any(|name| form == *name) {
                        return Err(RuntimeError::RangeError("invalid String normalization form".into()));
                    }
        ...
        Substr => {
            let start = integer(first)?;
            let start = (if start < 0.0 { len as f64 + start } else { start }).clamp(0.0, len as f64) as usize;
        ...
        Html { tag, attribute } => {
            ...
            if !attribute.is_empty() {
                ...
                for &unit in value.as_code_units() {
                    let escaped = if unit == u16::from(b'"') { "&quot;".into() } else { JsString::from_code_units(vec![unit]) };
...
fn unicode_transform(string: &JsString, form: &str, limit: usize) -> Result<JsString, RuntimeError> {
    let mut result = JsString::default();
    ...
    for scalar in char::decode_utf16(string.as_code_units().iter().copied()) {
        match scalar {
            ...
            Err(error) => {
                ...
                run.clear();
                append(&mut result, &JsString::from_code_units(vec![error.unpaired_surrogate()]), limit)?;
            }
        }
    }
    transform_run(&run, form, &mut result, limit)?;
...
fn transform_run(run: &str, form: &str, result: &mut JsString, limit: usize) -> Result<(), RuntimeError> {
