...
fn lexical_reads_typeof_writes_and_updates_are_in_the_tdz() {
    for source in [
        ...
        assert!(matches!(evaluate(source), Err(RuntimeError::ReferenceError(_))), "{source}");
    }
    assert_eq!(evaluate("x=missing; let x"), Err(RuntimeError::ReferenceError("missing".into())));
    assert_eq!(evaluate("x+=missing; let x"), Err(RuntimeError::ReferenceError("x".into())));
}
...
fn initialized_undefined_var_hoisting_and_loop_reentry_remain_distinct() {
    for (source, value) in [
        ...
        ("let n=0; for(let i=0;i<3;i++){let x=i; n+=x;} n", Value::Number(3.0)),
        ("let n=0; while(n<3){let x=n; n++; if(x<2)continue; break;} n", Value::Number(3.0)),
    ] {
...
fn tdz_failure_releases_roots_and_does_not_poison_reusable_bytecode() {
    let mut vm = Vm::new(VmConfig { heap: HeapConfig { nursery_capacity: 1, ..HeapConfig::default() }, ..VmConfig::default() }).unwrap();
    ...
    for _ in 0..3 {
        assert!(matches!(vm.execute(&failed), Err(RuntimeError::ReferenceError(_))));
...
fn nullish_mixing_requires_parentheses_at_the_current_grammar_level() {
    for source in ["1??2||3", "1||2??3", "1??2&&3", "1&&2??3", "1??2??3||4", "false&&(1??2||3)", "true?1:2??3||4", "`${1??2&&3}`"] {
...
fn number_literals_support_radices_separators_and_single_rounding() {
    for (source, expected) in [
        ...
        let Value::Number(actual) = evaluate(source).unwrap() else { panic!("expected number: {source}") };
...
fn malformed_numeric_tokens_are_rejected_before_parsing_statements() {
    for source in [
        "0x", "0b", "0o", "0b2", "0o8", "0xG", "0b12", "0o78", "1e", "1e+", "1.e-", "0_1", "00_1", "01_2", "08_9", "1__0", "1_", "1_.0", "1._0", "1_e2", "1e_2", "1e+_2", "1e2_", "0x_FF", "0b_1",
        "0o_7", "0xF_", "0o7__1", "123abc", "3in", "1$", "1\\u0061", "077e1", "077.5",
    ] {
        assert!(parse(source).is_err(), "{source}");
    }
...
fn all_four_line_terminators_drive_asi_and_end_line_comments() {
    for newline in ["\n", "\r", "\r\n", "\u{2028}", "\u{2029}"] {
        assert_eq!(evaluate(&format!("let x=1{newline}x+2")).unwrap(), Value::Number(3.0));
        assert_eq!(evaluate(&format!("let x=1// ignored{newline}x+2")).unwrap(), Value::Number(3.0));
        assert_eq!(evaluate(&format!("let x=1/*{newline}*/x+2")).unwrap(), Value::Number(3.0));
        assert_eq!(evaluate(&format!("let x=1; let y=2; x{newline}++y; y")).unwrap(), Value::Number(3.0));
...
fn quoted_strings_and_templates_apply_their_different_newline_rules() {
    for newline in ["\n", "\r", "\r\n"] {
        ...
        assert_eq!(evaluate(&format!("`a{newline}b`")).unwrap(), Value::String("a\nb".into()));
        assert_eq!(evaluate(&format!("`a\\{newline}b`")).unwrap(), Value::String("ab".into()));
    }
...
fn whitespace_uses_ecmascript_not_rust_unicode_classification() {
    for space in ['\t', '\u{000b}', '\u{000c}', ' ', '\u{00a0}', '\u{1680}', '\u{2000}', '\u{2007}', '\u{202f}', '\u{205f}', '\u{3000}', '\u{feff}'] {
        assert_eq!(evaluate(&format!("{space}1{space}+{space}2")).unwrap(), Value::Number(3.0));
    }
...
fn template_placeholder_comments_cannot_close_the_placeholder() {
    for newline in ["\n", "\r", "\r\n", "\u{2028}", "\u{2029}"] {
        assert_eq!(evaluate(&format!("`${{1 // }} ` ' ignored{newline}+2}}`")).unwrap(), Value::String("3".into()));
        assert_eq!(evaluate(&format!("`${{'a\\{newline}b'}}`")).unwrap(), Value::String("ab".into()));
    }
    assert_eq!(evaluate("`${1/* } { ` ' */+2}`").unwrap(), Value::String("3".into()));
    assert_eq!(evaluate("`${`nested ${1/* } */+2}`}`").unwrap(), Value::String("nested 3".into()));
