...
use blueice_bluejs::{
    compile, parse, Heap, HeapConfig, HeapError, JsString, RuntimeError, Value, Vm, VmConfig,
};
...
fn numeric_coercion_does_not_repair_surrogates() {
    for source in [
        ...
        assert!(
            matches!(evaluate(source), Value::Number(n) if n.is_nan()),
            "{source}"
        );
    }
...
fn host_strings_and_property_keys_are_lossless_and_exactly_accounted() {
    ...
    heap.set(object, &string, Value::String("😀".into()))
        .unwrap();
...
fn surrogate_key_edges_survive_promotion_and_are_released_on_deletion() {
    ...
    heap.set(
        owner,
        JsString::from_code_units(vec![0xd801]),
        Value::Object(second),
    )
    .unwrap();
    heap.set(owner, "4294967296", Value::Number(8.0)).unwrap();
    heap.set(owner, "9999999999999999999999999999999", Value::Number(9.0))
        .unwrap();
    heap.collect_major();
...
fn boxed_string_virtual_properties_have_their_required_storage_invariants() {
    ...
    assert_eq!(
        heap.set(object, "0", Value::Undefined),
        Err(HeapError::ReadOnlyProperty)
    );
    assert_eq!(
        heap.set(object, "length", Value::Number(0.0)),
        Err(HeapError::ReadOnlyProperty)
    );
