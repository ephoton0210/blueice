...
fn compilation_emits_a_stack_program_with_fixed_width_operands() {
    ...
    assert_eq!(
        ...
        [Opcode::EnterScope, Opcode::Constant, Opcode::Constant, Opcode::Constant, Opcode::Multiply, Opcode::Add, Opcode::SetCompletion, Opcode::Halt]
    );
    let mut offset = 0;
    for instruction in instructions {
        ...
        if let Some(operand) = instruction.operand {
            assert_eq!(&code.bytes()[offset + 1..offset + 5], &operand.to_le_bytes());
        }
        offset += instruction.opcode.width();
    }
    assert_eq!(offset, code.bytes().len());
    assert_eq!(code.constants(), &[Value::Number(1.0), Value::Number(2.0), Value::Number(3.0)]);
    assert_eq!(Opcode::GetProperty.flags(), blueice_bluejs::MAY_USE_INLINE_CACHE);
...
fn numeric_strings_use_js_grammar_whitespace_and_single_rounding() {
    ...
    for input in [".", "+", "1e", "1e+", "1.2x", "１", "0x", "0b2", "-0x10", "+0o2", "inf", "\u{85}"] {
        assert!(matches!(evaluate(&format!("+'{input}'")).unwrap(), Value::Number(n) if n.is_nan()), "{input}");
    }
...
fn primitive_string_formatting_and_utf16_comparison_cover_boundaries() {
    for (source, expected) in [
        ...
        assert_eq!(evaluate(source).unwrap(), Value::String(expected.into()), "{source}");
    }
    assert_eq!(evaluate("'😀' < '￿' && '😀' > 'a' && !!{}").unwrap(), Value::Bool(true));
...
fn every_compound_store_and_prototype_literal_form_executes() {
    for (source, expected) in [
        ...
        assert_eq!(evaluate(source).unwrap(), Value::Number(expected), "{source}");
    }
...
fn heap_errors_release_temporary_roots_and_previous_results_even_on_failure() {
    let mut vm = Vm::new(VmConfig { heap: HeapConfig { nursery_capacity: 1, major_threshold_bytes: 1024, max_heap_bytes: 2048 }, ..VmConfig::default() }).unwrap();
    let baseline = vm.heap().stats().managed_bytes;
    for source in ["let o={}; while(true){o={next:o};}", "let o={}; let s='xxxxxxxx'; for(let i=0;i<9;i++){s+=s;} o.x=s;"] {
        let result = vm.execute(&compile(&parse("({keep:{}})").unwrap()).unwrap()).unwrap();
        let Value::Object(old) = result else { panic!("expected object") };
        let error = vm.execute(&compile(&parse(source).unwrap()).unwrap()).unwrap_err();
        assert_eq!(error, RuntimeError::Heap(blueice_bluejs::HeapError::HeapLimitExceeded { limit: 2048 }));
        assert!(std::error::Error::source(&error).is_some());
        assert!(!error.to_string().is_empty());
        assert_eq!(vm.heap().stats().managed_bytes, baseline, "failed safepoint leaked a root");
    ...
    let Value::Object(object) = vm.execute(&compile(&parse("({keep:{x:7}}); let garbage={};").unwrap()).unwrap()).unwrap() else { panic!("expected completion") };
    assert!(matches!(vm.heap().get(object, "keep").unwrap(), Value::Object(_)));
}
...
fn configuration_boundaries_and_error_messages_are_observable() {
    ...
    let mut vm = Vm::new(VmConfig { instruction_budget: 0, ..VmConfig::default() }).unwrap();
    assert_eq!(vm.execute(&compile(&parse("").unwrap()).unwrap()), Err(RuntimeError::InstructionLimit));
    let mut vm = Vm::new(VmConfig { max_string_bytes: 3, ..VmConfig::default() }).unwrap();
...
fn hand_built_invalid_ast_returns_compile_errors_and_code_is_reusable() {
    ...
    for expr in [
        Expr::Template { quasis: vec![], expressions: vec![] },
        Expr::Member { object: Box::new(Expr::Null), property: Box::new(Expr::Null), computed: false },
        Expr::Assign { op: blueice_bluejs::AssignOp::Assign, target: Box::new(Expr::Null), value: Box::new(Expr::Null) },
    ] {
        assert!(matches!(compile(&Program { body: vec![Stmt::Expr(expr)] }), Err(CompileError::InvalidSyntax(_))));
    }
    ...
    let Value::Object(third) = a.execute(&code).unwrap() else { panic!("expected object") };
    assert_ne!(first, Value::Object(third));
    assert_eq!(a.heap().get(third, "x").unwrap(), Value::Number(2.0));
    let loops = compile(&parse("for(let i=0;i<2;i++){if(i)continue;else break;}").unwrap()).unwrap();
    let loop_offsets: std::collections::HashSet<_> = loops.instructions().map(|i| i.offset).collect();
    for instruction in loops.instructions() {
        if matches!(instruction.opcode, Opcode::Jump | Opcode::JumpIfFalse | Opcode::JumpIfTrue | Opcode::JumpIfNotNullish) {
...
fn arithmetic_and_primitive_coercions_execute_with_js_behavior() {
    for (source, value) in [
        ...
        ("!0 && !'' && !null && !undefined && !NaN", Value::Bool(true)),
        ...
        ("`a${1 + 2}:${false}:${null}`", Value::String("a3:false:null".into())),
    ] {
...
fn short_circuiting_and_conditionals_skip_unevaluated_branches() {
    for (source, value) in [
        ...
        ("let x = 0; false && x++; true || x++; 1 ?? x++; x", Value::Number(0.0)),
    ] {
        assert_eq!(evaluate(source).unwrap(), value, "{source}");
    }
}
...
fn loops_unwind_nested_scopes_on_break_and_continue() {
    ...
    assert!(matches!(evaluate("for(let i=0;i<1;i++){} i"), Err(RuntimeError::ReferenceError(_))));
}
...
fn object_operations_preserve_identity_and_evaluation_order() {
    for (source, result) in [
        ...
        ("let k='x'; let o={ [k]:3, k }; o.x + o.k", Value::String("3x".into())),
        ("let i=0; let o={0:3}; let a=o[i++]++; a*100+o[0]*10+i", Value::Number(341.0)),
        ("let i=0; let o={0:3}; o[i++] += (i=5); o[0]*10+i", Value::Number(85.0)),
        ("let o={x:'2'}; let a=++o.x; let b=o.x--; a*100+b*10+o.x", Value::Number(332.0)),
        ("let x='2'; let a=x++; let b=--x; a*100+b*10+x", Value::Number(222.0)),
...
fn runtime_errors_and_limits_leave_the_vm_reusable() {
    ...
    assert_eq!(vm.execute(&compile(&parse("for(;;){}").unwrap()).unwrap()), Err(RuntimeError::InstructionLimit));
    assert_eq!(vm.execute(&compile(&parse("let s='x';while(true){s+=s;}").unwrap()).unwrap()), Err(RuntimeError::StringLimit { limit: 128 }));
}

#[test]
fn objects_on_the_stack_and_in_bindings_survive_gc_and_results_live_until_the_next_run() {
    let mut vm = Vm::new(VmConfig { heap: HeapConfig { nursery_capacity: 1, major_threshold_bytes: 1024, max_heap_bytes: 8192 }, ..VmConfig::default() }).unwrap();
    let code = compile(&parse("let keep={x:4}; let o={a:{b:{c:7}}, d:{x:8}}; for(let i=0;i<40;i++){let garbage={n:i};} o.keep=keep; o").unwrap()).unwrap();
    let Value::Object(object) = vm.execute(&code).unwrap() else { panic!("expected object") };
    let Value::Object(keep) = vm.heap().get(object, "keep").unwrap() else { panic!("expected child") };
    assert_eq!(vm.heap().get(keep, "x").unwrap(), Value::Number(4.0));
    let Value::Object(a) = vm.heap().get(object, "a").unwrap() else { panic!("expected child") };
    let Value::Object(b) = vm.heap().get(a, "b").unwrap() else { panic!("expected child") };
    assert_eq!(vm.heap().get(b, "c").unwrap(), Value::Number(7.0));
    assert!(vm.heap().stats().minor_collections > 1);
    assert_eq!(vm.execute(&compile(&parse("1").unwrap()).unwrap()).unwrap(), Value::Number(1.0));
