...
fn sparse_arrays_distinguish_holes_and_length_from_ordinary_properties() {
    ...
    assert_eq!(heap.get_own(array, "length").unwrap(), Some(Value::Number(3.0)));
    assert_eq!(heap.get_own(array, "0").unwrap(), None);
    heap.set(array, "1", Value::Undefined).unwrap();
    heap.set(array, "name", Value::String("array".into())).unwrap();
    ...
    heap.set(ordinary, "length", Value::String("ordinary".into())).unwrap();
...
fn canonical_index_boundaries_grow_length_without_dense_allocation() {
    ...
    heap.set(array, "length", Value::Number(u32::MAX as f64)).unwrap();
    ...
    for key in ["01", "-0", "+1", "1.0", "4294967295"] {
        ...
        assert_eq!(heap.get(array, "length").unwrap(), Value::Number(0.0), "{key}");
    }
    heap.set(array, "4294967294", Value::Number(7.0)).unwrap();
    assert_eq!(heap.get(array, "length").unwrap(), Value::Number(u32::MAX as f64));
    assert!(heap.stats().managed_bytes < baseline + 2048);
    heap.set(array, "0", Value::Number(1.0)).unwrap();
    assert_eq!(heap.own_keys(array).unwrap(), ["0", "4294967294", "length", "01", "-0", "+1", "1.0", "4294967295"]);
    heap.set(array, "length", Value::Number(-0.0)).unwrap();
    let Value::Number(length) = heap.get(array, "length").unwrap() else { panic!("numeric length") };
    assert_eq!(length.to_bits(), 0.0f64.to_bits());
    assert_eq!(heap.own_keys(array).unwrap(), ["length", "01", "-0", "+1", "1.0", "4294967295"]);
}

#[test]
fn invalid_length_and_failed_index_stores_are_atomic() {
    let mut heap = Heap::new(HeapConfig { nursery_capacity: 1, major_threshold_bytes: 1024, max_heap_bytes: 2048 }).unwrap();
    let array = heap.alloc_array(2, None).unwrap();
    heap.root(array).unwrap();
    heap.set(array, "1", Value::Number(9.0)).unwrap();
    let baseline = heap.stats().managed_bytes;
    for value in [Value::Number(-1.0), Value::Number(1.5), Value::Number(f64::NAN), Value::Number(f64::INFINITY), Value::Number(4294967296.0), Value::String("2".into())] {
        assert_eq!(heap.set(array, "length", value), Err(HeapError::InvalidArrayLength));
...
fn truncation_removes_gc_edges_and_preserves_inherited_and_named_properties() {
    ...
    heap.set(array, "name", Value::String("keep".into())).unwrap();
    heap.collect_minor();
    ...
    assert_eq!(heap.get_own(array, "1").unwrap(), None);
    assert_eq!(heap.get(array, "name").unwrap(), Value::String("keep".into()));
    heap.collect_major();
    assert!(!heap.contains(child));
...
fn literals_holes_index_updates_and_length_coercions_execute() {
    for (source, expected) in [
        ...
        ("let a=[2,4,6]; let s=0; for(let i=0;i<a.length;i++){s+=a[i];} s", Value::Number(12.0)),
        ("let a=[]; a[4]=7; a.length*10+a[4]", Value::Number(57.0)),
        ("let a=[1,2,3]; a.length=1; a[2]", Value::Undefined),
        ("let a=[1]; let result=(a.length='3'); result=== '3' && a.length===3", Value::Bool(true)),
        ...
        ("let a=[]; a.length=-0; 1/a.length", Value::Number(f64::INFINITY)),
        ("let a=[1,2,3]; let n=a.length--; n*10+a.length", Value::Number(32.0)),
        ("let a=[]; a.length+='2'; a.length", Value::Number(2.0)),
        ("let i=0; let a=[i++,i++,i++]; a[0]*100+a[1]*10+a[2]", Value::Number(12.0)),
        ("let i=0; let a=[3]; let old=a[i++]++; old*100+a[0]*10+i", Value::Number(341.0)),
    ] {
...
fn nested_arrays_survive_vm_safepoints_and_share_an_array_prototype() {
    ...
    let code = compile(&parse("let a=[{x:7},[8],,undefined]; a[4]=a; for(let i=0;i<40;i++){let garbage=[{},[]];} a").unwrap()).unwrap();
    let Value::Object(array) = vm.execute(&code).unwrap() else { panic!("expected array") };
    assert!(vm.heap().is_array(array).unwrap());
    assert_eq!(vm.heap().get_own(array, "2").unwrap(), None);
    assert_eq!(vm.heap().get_own(array, "3").unwrap(), Some(Value::Undefined));
    assert_eq!(vm.heap().get(array, "4").unwrap(), Value::Object(array));
    let Value::Object(object) = vm.heap().get(array, "0").unwrap() else { panic!("expected object element") };
    assert_eq!(vm.heap().get(object, "x").unwrap(), Value::Number(7.0));
    let Value::Object(child) = vm.heap().get(array, "1").unwrap() else { panic!("expected child array") };
    ...
    assert_eq!(vm.heap().get(prototype, "length").unwrap(), Value::Number(0.0));
...
fn array_entry_points_reject_stale_and_cross_heap_handles() {
    ...
    for id in [stale, foreign] {
        ...
        assert_eq!(heap.enumerable_own_keys(id), Err(HeapError::InvalidObject(id)));
        assert_eq!(heap.alloc_array(0, Some(id)), Err(HeapError::InvalidObject(id)));
        assert_eq!(heap.set(id, "length", Value::Number(1.0)), Err(HeapError::InvalidObject(id)));
        assert_eq!(heap.set(array, "3", Value::Object(id)), Err(HeapError::InvalidObject(id)));
        assert_eq!(heap.set(array, "length", Value::Object(id)), Err(HeapError::InvalidObject(id)));
    }
    assert_eq!(heap.get(array, "length").unwrap(), Value::Number(2.0));
...
fn minor_gc_does_not_retain_truncated_or_replaced_array_elements() {
    let mut heap = Heap::new(HeapConfig { nursery_capacity: 8, ..HeapConfig::default() }).unwrap();
    let array = heap.alloc_array(3, None).unwrap();
    heap.root(array).unwrap();
    heap.collect_minor();
...
fn sparse_mutations_match_an_independent_optional_slot_model() {
    let mut heap = Heap::new(HeapConfig { nursery_capacity: 2, major_threshold_bytes: 2048, max_heap_bytes: 8192 }).unwrap();
    let array = heap.alloc_array(0, None).unwrap();
    heap.root(array).unwrap();
    heap.set(array, "tag", Value::String("kept".into())).unwrap();
    let baseline = heap.stats().managed_bytes;
    ...
    for step in 0..256 {
        ...
        match state % 4 {
            0 => {
                heap.set(array, "length", Value::Number(index as f64)).unwrap();
    ...
    heap.collect_major();
    assert_eq!(heap.stats().managed_bytes, baseline);
    assert_eq!(heap.get(array, "tag").unwrap(), Value::String("kept".into()));
}
...
fn new_array_encoding_and_runtime_error_boundaries_are_observable() {
    ...
    assert!(HeapError::InvalidArrayLength.to_string().contains("array length"));

    let mut probe = Heap::default();
    probe.alloc_object(None).unwrap();
    let one_object = probe.stats().managed_bytes;
    assert!(matches!(
        Vm::new(VmConfig { heap: HeapConfig { nursery_capacity: 1, major_threshold_bytes: one_object, max_heap_bytes: one_object }, ..VmConfig::default() }),
    ...
    assert_eq!(vm.execute(&compile(&parse("[7][0]").unwrap()).unwrap()).unwrap(), Value::Number(7.0));
}
