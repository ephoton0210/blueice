...
fn adapter(requests: &[Value], fault: Option<(&str, &std::path::Path)>) -> Vec<Value> {
    ...
    if let Some((mode, path)) = fault {
        command.env("BLUEJS_REGEXP_WORKER", path).env("BLUEJS_FAULT_MODE", mode);
    }
...
fn adapter_preserves_phases_limits_and_fresh_realms() {
    let cases = [
        ...
        (json!({"source":"assert.sameValue(typeof x,'undefined')", "mode":"strict"}), "ok"),
        (json!({"source":"assert(true)", "mode":"raw"}), "ReferenceError"),
        (json!({"source":"$262.createRealm()", "mode":"raw"}), "unsupported"),
        (json!({"source":"1", "mode":"module"}), "unsupported"),
        (json!({"source":"1", "mode":"sloppy", "asynchronous":true}), "unsupported"),
        (json!({"source":"let x;let x;", "mode":"sloppy", "parse_only":true}), "SyntaxError"),
        (json!({"source":"let =", "mode":"sloppy", "parse_only":true}), "unclassified_parse_error"),
        (json!({"source":"try{}catch([...rest, next]){}", "mode":"sloppy", "parse_only":true}), "SyntaxError"),
        (json!({"source":"try{}catch({...rest, next}){}", "mode":"sloppy", "parse_only":true}), "SyntaxError"),
        (json!({"source":"try{}catch(eval){}", "mode":"strict", "parse_only":true}), "SyntaxError"),
        (json!({"source":"try{}", "mode":"sloppy", "parse_only":true}), "SyntaxError"),
        (json!({"source":"catch", "mode":"sloppy", "parse_only":true}), "SyntaxError"),
        (json!({"source":"try{}finally(){}", "mode":"sloppy", "parse_only":true}), "SyntaxError"),
        (json!({"source":"1", "mode":"sloppy", "parse_only":true}), "ok"),
        ...
        (json!({"source":"1", "mode":"sloppy", "includes":["helpers.js"], "harness_sources":["try{}catch(e){}"]}), "ok"),
        ...
        (json!({"source":"'a'.repeat(-1)", "mode":"sloppy"}), "RangeError"),
        ...
        (json!({"source":"1", "mode":"raw", "includes":["other.js"]}), "ok"),
...
fn regex_protocol_faults_and_missing_helper_fail_closed() {
    ...
    for mode in ["early_exit", "bad_ready", "after_ready_exit", "malformed", "oversized", "invalid_range", "compile_reply", "match_reply"] {
        let replies = adapter(&[json!({"source":"new RegExp('a').test('a')", "mode":"sloppy"})], Some((mode, &script)));
        assert_eq!(replies[0]["kind"], "worker_error", "{mode}: {replies:?}");
    }
    let missing = directory.join("missing");
    for source in ["/a/", "new RegExp('a')"] {
        let replies = adapter(&[json!({"source":source,"mode":"sloppy"})], Some(("unused", &missing)));
...
fn helper_rejects_oversized_frames_and_handles_out_of_range_starts() {
    let mut child = Command::new(env!("CARGO_BIN_EXE_bluejs-regexp-worker")).stdin(Stdio::piped()).stdout(Stdio::piped()).stderr(Stdio::null()).spawn().unwrap();
    let mut input = child.stdin.take().unwrap();
    ...
    let request = serde_json::to_vec(&json!({"source":[97],"flags":"","input":[97],"start":2})).unwrap();
    input.write_all(&(request.len() as u32).to_le_bytes()).unwrap();
    input.write_all(&request).unwrap();
    input.flush().unwrap();
    assert_eq!(serde_json::from_slice::<Value>(&read_frame(&mut output)).unwrap(), json!({"Found":null}));
    input.write_all(&(16 * 1024 * 1024u32 + 1).to_le_bytes()).unwrap();
    drop(input);
