...
fn direct_eval_uses_the_callers_bindings_completion_and_strictness() {
    ...
    for source in [
        ...
        "function withAssignmentReference(){var value=0;let object={value:1};with(object){value=(delete object.value,2);}return object.value===2&&value===0;}withAssignmentReference()",
        "function withCompoundAssignmentReference(){let object={value:1};with(object){value+=(delete object.value,2);}return object.value===3;}withCompoundAssignmentReference()",
        "function withCompoundAssignmentBinding(){let value=1;with({}){value+=2;}return value===3;}withCompoundAssignmentBinding()",
        "function callAssignmentTarget(){let calls=0;let rhs=false;function f(){calls++;return {valueOf(){throw Error('coercion')}}}function g(){rhs=true;}let errors=0;for(let action of [()=>f()=g(),()=>f()+=g(),()=>f()++,()=>++f(),()=>{for(f() of [1]){}}]){try{action()}catch(error){if(error instanceof ReferenceError)errors++;}}return calls===5&&!rhs&&errors===5;}callAssignmentTarget()",
        "function async(){}let errors=0;for(let action of [()=>async()=1,()=>async()+=1,()=>async()++,()=>++async()]){try{action()}catch(error){if(error instanceof ReferenceError)errors++;}}errors===4",
        ...
        "function directEvalVarShadowsCapture(){var x=0;let inner=(function(){x=(eval('var x;'),1);return x;})();return inner===undefined&&x===1;}directEvalVarShadowsCapture()",
        "function dynamicEvalAssignmentAfterResolution(){var x=0;let inner=()=>{eval('var x;');x=2;return x===2;};return inner()&&x===0;}dynamicEvalAssignmentAfterResolution()",
...
fn generators_suspend_resume_and_close_as_iterators() {
    ...
    for source in [
        ...
        "let iter=(function*(){let value=yield;return value;})();let first=iter.next();let done=iter.next(7);first.value===undefined&&!first.done&&done.value===7&&done.done",
        "let closed=0;let iterator={next(){return {value:undefined,done:false};},return(){closed++;return {};}};let iterable={[Symbol.iterator](){return iterator;}};let iter=(function*(){let target;[target=yield]=iterable;})();iter.next();let done=iter.return(7);closed===1&&done.value===7&&done.done",
