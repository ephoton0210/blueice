...
fn locale_casing_and_canonicalization() {
    for source in [
        ...
        "let currencies=Intl.supportedValuesOf('currency');currencies.join()==='EUR,JPY,USD'&&currencies.every(currency=>typeof new Intl.DisplayNames('en',{type:'currency',fallback:'none'}).of(currency)==='string')",
...
fn duration_format_delegates_records_options_and_parts_to_the_host_service() {
    for source in [
        ...
        "Intl.DurationFormat.supportedLocalesOf(['en','zz','es']).join(',') === 'en' && new Intl.DurationFormat('es').resolvedOptions().locale === 'en-US' && Object.prototype.toString.call(new Intl.DurationFormat()) === '[object Intl.DurationFormat]'",
