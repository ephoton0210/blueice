...
A mode's status is one of `pass`, `fail`, `timeout`, `harness_error`, `unsupported` or `excluded`. `unsupported` means the adapter tried the fixture and reported a capability BlueJS's engine genuinely lacks (a missing host hook, an unsupported harness include). `excluded` is a different, narrower thing: it means the fixture's own Test262 metadata declares it applicable only to a host with the opposite of a capability this host has deliberately chosen, so the runner never dispatches it to the adapter at all. The only current case is `CanBlockIsTrue`/`CanBlockIsFalse` (`HOST_CAN_BLOCK` in `run.py`): BlueJS's `Atomics.wait` genuinely suspends the agent, so every `CanBlockIsTrue` fixture is dispatched and already passes, while a `CanBlockIsFalse` fixture is excluded — making it pass instead would mean making `Atomics.wait` always throw, which would break every `CanBlockIsTrue` fixture. Both statuses still count as non-passing for the exit code above; the distinction is for triage (`TEST262_ANALYSIS_REPORT.md`'s `host-capability-declared` blocker vs. `compiler-unsupported`), not for hiding either from the inventory.
