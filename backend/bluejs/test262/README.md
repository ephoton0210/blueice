...
This POSIX runner inventories every test in the pinned official snapshot, including staging/proposals and ECMA-402. It reports every requested execution mode. **A full inventory run is not a passing conformance result.** Current module/async hosts, additional harness scripts and much of the language remain unsupported.
...
Outputs are `target/test262/results.jsonl` (one record per test/mode, source hash, features, expected and actual phase/type/status) and `summary.json` (counts and feature/group breakdown). `--filter intl402/Collator` runs a clearly labelled partial inventory. `--jobs` defaults to 8 and `--timeout` to 2 seconds. Exit 0 requires every scheduled mode to pass; exit 1 means non-passing cases and exit 2 indicates a configuration/argument error. Unexpected Python errors terminate with a traceback and must not be treated as a completed report.

The adapter creates a fresh VM for each case. Raw source is unchanged and receives no assertion harness. Other cases receive native overrides for `sta.js` and `assert.js`, permitted by the pinned `INTERPRETING.md`. Harness assertions fail closed, compare signed zero and NaN correctly, and keep resource failures distinct from JavaScript exceptions. Additional includes are validated, then reported unsupported: concatenating their source would incorrectly change strictness and global lexical scoping. Module and async modes likewise remain visible as unsupported. Missing `$262`, `$DONE` and `print` hooks cannot satisfy negative ReferenceError tests. The subset parser's unclassified rejections never count as successful negative SyntaxError tests. Arbitrary thrown values currently report `ThrownValue`; they are not guessed to have the expected error class.
...
The checked-in [summary](../../../development/browser_core/phase-13-bluejs-engine/test262-summary.json) records the current complete inventory. Full passing conformance, a persistent multi-script global environment, complete host hooks, and the remaining ECMA-402 constructors are still open work.
