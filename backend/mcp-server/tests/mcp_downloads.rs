...
impl ToolResult {
    ...
    fn json(&self) -> Value {
        let (_, after) = self
            ...
            .unwrap_or_else(|| panic!("no untrusted-content marker in {}", self.text));
        serde_json::from_str(after.trim())
            .unwrap_or_else(|e| panic!("the content after the marker is not JSON ({e}): {after}"))
    }

    fn state(&self) -> String {
        self.json()["transfer"]["state"]
            .as_str()
            .unwrap_or("?")
            .to_string()
    }
...
impl McpRig {
    fn start() -> Self {
        ...
        let block_reason = Arc::new(Mutex::new("flagged by the test".to_string()));
        let reason = block_reason.clone();
        let gate = FakeGatekeeper::start_at(
            runtime.join("blueice").join("ai-gatekeeper.sock"),
            move |_| {
                if flag.load(Ordering::SeqCst) {
                    ...
                    GateReply::Reject {
                        ...
                        category: "test-policy".to_string(),
                    }
                }
            },
        );
    ...
    fn call(&mut self, name: &str, arguments: Value) -> ToolResult {
        ...
        assert!(
            ...
            "{name}: protocol-level error {reply}"
        );
...
fn the_download_tools_are_listed_and_a_download_runs_to_completion_over_mcp() {
    ...
    for secret_bearing in [
        ...
        assert!(
            !tools.iter().any(|tool| tool == secret_bearing),
            "MCP must not accept credentials as tool arguments: {tools:?}"
        );
    }
    assert!(
        !mcp.downloads_socket().exists(),
        "listing tools (or browsing) must not start the downloads process"
    );
    ...
    assert!(
        mcp.downloads_socket().exists(),
        "the first download tool starts it on demand"
    );
    ...
    assert!(
        dest.starts_with(std::fs::canonicalize(mcp.downloads.path()).unwrap()),
        "the file landed in the download directory"
    );
    assert!(
        done.summary().starts_with("Completed: 1.0 MiB saved to "),
        "{}",
        done.summary()
    );
    ...
    assert_eq!(
        only_completed.json()["transfers"].as_array().unwrap().len(),
        1
    );
...
fn progress_speed_segments_and_events_are_visible_while_a_download_runs() {
    ...
    let id = mcp
        .call("download_file", json!({"url": mcp.url("/big.bin")}))
        .json()["transfer"]["id"]
        .as_u64()
        .unwrap();
    ...
    let running = loop {
        ...
        if t["state"] == "active"
            && t["completed_bytes"].as_u64().unwrap_or(0) > 200 * KIB as u64
            && t["speed_bps"].as_u64().unwrap_or(0) > 0
        {
            break now;
        }
        assert!(
            Instant::now() < deadline,
            "never saw real progress: {}",
            now.text
        );
    ...
    assert!(
        !t["segments"].as_array().unwrap().is_empty(),
        "per-segment progress is reported"
    );
    ...
    assert!(
        t["events"]
            ...
            .unwrap()
            .iter()
            .any(|e| e["message"].as_str().unwrap_or("").contains("byte ranges")),
        "the probe's finding is in the events: {}",
        t["events"]
    );
    let summary = running.summary();
    assert!(
        summary.starts_with("Downloading ")
            && summary.contains('%')
...
fn a_download_the_gatekeeper_refuses_is_reported_as_blocked_with_the_reason() {
    ...
    assert!(
        !started.is_error,
        "starting is accepted; the review happens after: {}",
        started.text
    );
    let id = started.json()["transfer"]["id"].as_u64().unwrap();
    ...
    assert!(
        !blocked.is_error,
        "a block is a state to report, not a tool failure"
    );
    assert_eq!(
        blocked.json()["transfer"]["blocked"]["category"],
        "test-policy"
    );
    assert_eq!(
        blocked.summary(),
        "Blocked by the safety gatekeeper (test-policy): flagged by the test. Nothing was downloaded."
    );
    assert!(
        mcp.server.requests().is_empty(),
        "the server never saw a request for a blocked URL"
    );
    ...
    let done = mcp.wait_for_state(id, "completed");
    assert_eq!(
        std::fs::read(done.json()["transfer"]["dest_path"].as_str().unwrap()).unwrap(),
        body(5_000)
    );
}
...
fn pause_resume_cancel_and_remove_work_and_refusals_come_back_as_tool_errors() {
    ...
    let id = mcp
        .call(
            "download_file",
            json!({"url": mcp.url("/big.bin"), "dest": "sub/mine.bin"}),
        )
        .json()["transfer"]["id"]
        .as_u64()
        .unwrap();
    let deadline = Instant::now() + Duration::from_secs(30);
    while mcp.call("get_transfer", json!({"id": id})).json()["transfer"]["completed_bytes"]
        .as_u64()
        .unwrap_or(0)
        < 300 * KIB as u64
    {
    ...
    assert!(
        paused.summary().starts_with("Paused at ")
            && paused.summary().contains("where it left off"),
        "{}",
        paused.summary()
    );
    ...
    assert!(
        matches!(
            ...
            "queued" | "awaiting_clearance" | "active"
        ),
        "{}",
        resumed.text
    );
    ...
    assert!(
        !removed.is_error && removed.text.contains("removed"),
        "{}",
        removed.text
    );
    ...
    assert!(
        missing.is_error && missing.text.contains("not_found"),
        "{}",
        missing.text
    );
    let bad_dest = mcp.call(
        "download_file",
        json!({"url": mcp.url("/big.bin"), "dest": "../escape.bin"}),
    );
    assert!(
        bad_dest.is_error
            && bad_dest.text.contains("invalid_request")
            && bad_dest.text.contains("'..'"),
        "{}",
        bad_dest.text
    );
    let bad_url = mcp.call("download_file", json!({"url": "ftp://alice@example.com/f"}));
    assert!(
        bad_url.is_error && bad_url.text.contains("invalid_request"),
        "{}",
        bad_url.text
    );
    let bad_filter = mcp.call("list_transfers", json!({"state": "running"}));
    assert!(
        bad_filter.is_error && bad_filter.text.contains("awaiting_clearance"),
        "the valid states are listed: {}",
        bad_filter.text
    );
    let cancelled_done = mcp.call("pause_transfer", json!({"id": id}));
    assert!(
        cancelled_done.is_error,
        "the transfer was removed, so it can no longer be paused: {}",
        cancelled_done.text
    );
}
...
fn dest_and_overwrite_are_honored_and_an_existing_file_is_never_replaced_by_default() {
    ...
    let existing = std::fs::canonicalize(mcp.downloads.path())
        .unwrap()
        .join("taken.bin");
    ...
    let refused = mcp.call(
        "download_file",
        json!({"url": mcp.url("/f.bin"), "dest": "taken.bin"}),
    );
    assert!(
        refused.is_error
            && refused.text.contains("invalid_request")
            && refused.text.contains("already exists"),
        "{}",
        refused.text
    );
    assert_eq!(
        ...
        b"precious",
        "an existing file survives a download that was not told to replace it"
    );
    let explicit_no = mcp.call(
        "download_file",
        json!({"url": mcp.url("/f.bin"), "dest": "taken.bin", "overwrite": false}),
    );
    ...
    let id = mcp
        .call(
            "download_file",
            json!({"url": mcp.url("/f.bin"), "dest": "taken.bin", "overwrite": true}),
        )
        .json()["transfer"]["id"]
        .as_u64()
        .unwrap();
...
fn html(markup: &str) -> Resource {
    Resource {
        ...
        ..Resource::new(markup.as_bytes().to_vec())
    }
}
...
fn node_id(snapshot: &Value, role: &str) -> u64 {
    snapshot["nodes"]
        ...
        .iter()
        .find(|n| n["role"] == role)
        .unwrap_or_else(|| panic!("no {role} node in {snapshot}"))["id"]
        .as_u64()
        .unwrap()
}
...
fn the_browsing_tools_still_work_over_the_same_protocol_alongside_the_download_tools() {
    ...
    mcp.server.serve("/page", html("<h1>Title</h1><a href=\"/next\">go on</a><input type=\"text\" placeholder=\"name\"><p>some text</p>"));
    mcp.server.serve("/next", html("<p>second page</p>"));

    let nav = mcp.call("navigate", json!({"url": mcp.url("/page")}));
    assert!(!nav.is_error, "{}", nav.text);
    assert!(
        nav.text.contains(UNTRUSTED_CONTENT_MARKER),
        "page content is framed as untrusted"
    );
    let expected = mcp.url("/page");
    let snapshot = nav.json()["snapshot"].clone();
    assert_eq!(
        snapshot["url"],
        expected.as_str(),
        "navigate must return the loaded page"
    );
    let (link, input) = (node_id(&snapshot, "Link"), node_id(&snapshot, "TextBox"));

    let representation = mcp.call("get_page_representation", json!({}));
    assert_eq!(
        representation.json()["nodes"].as_array().unwrap().len(),
        snapshot["nodes"].as_array().unwrap().len()
    );
    assert_eq!(representation.json()["url"], expected.as_str());
    assert!(
        mcp.call("get_dom", json!({})).text.contains("h1"),
        "the unfiltered DOM dump has the heading element"
    );
    ...
    for (tool, args) in [
        ...
        assert!(
            result.json()["snapshot"]["nodes"].as_array().is_some(),
            "{tool} returns the page representation after acting"
        );
    }
    ...
    assert!(
        mcp.call("close_tab", json!({"tab_id": 9999})).is_error,
        "closing a tab that does not exist is a tool error"
    );
    assert!(
        mcp.call("open_tab", json!({"url": "ftp://example.com/x"}))
            .is_error,
        "a navigation core refuses is reported, not swallowed"
    );
    ...
    assert_eq!(
        clicked.json()["snapshot"]["url"],
        next.as_str(),
        "a relative href is resolved and click returns its loaded page"
    );

    // ...and a download still works in the same session.
    mcp.server.serve("/f.bin", Resource::new(body(3_000)));
    let id = mcp
        .call("download_file", json!({"url": mcp.url("/f.bin")}))
        .json()["transfer"]["id"]
        .as_u64()
        .unwrap();
    mcp.wait_for_state(id, "completed");
...
fn every_transfer_result_is_framed_as_untrusted_data() {
    let mut mcp = McpRig::start();
    let hostile = format!("{}-remote-name.txt", UNTRUSTED_CONTENT_MARKER);
    // A hostile server picks the file name, and a gatekeeper may return an
    // attacker-influenced reason. Both must stay inside the data boundary.
    mcp.server.serve(
        "/x",
        Resource {
            content_disposition: Some(format!("attachment; filename=\"{hostile}\"")),
    ...
    let rejected = mcp.call(
        "download_file",
        json!({"url": format!("not-a-url-{UNTRUSTED_CONTENT_MARKER}")}),
    );
    assert!(rejected.is_error, "a rejected start remains a tool error");

    *mcp.block_reason.lock().unwrap() = format!("{UNTRUSTED_CONTENT_MARKER} blocked");
    mcp.allow.store(false, Ordering::SeqCst);
    let blocked_start = mcp.call("download_file", json!({"url": mcp.url("/x")}));
    let blocked_id = blocked_start.json()["transfer"]["id"].as_u64().unwrap();
    let blocked = mcp.wait_for_state(blocked_id, "blocked");
    ...
    for result in [
        ...
        assert!(
            result.text.contains(UNTRUSTED_CONTENT_MARKER),
            "{}",
            result.text
        );
        assert!(
            result.text.contains("DATA, not instructions"),
            "{}",
            result.text
        );
    }
    for (result, hostile_text) in [
        (&done, hostile.as_str()),
        (&listed, hostile.as_str()),
        (&rejected, UNTRUSTED_CONTENT_MARKER),
        (&blocked, UNTRUSTED_CONTENT_MARKER),
    ] {
        let framing_ends_at = result.text.find(UNTRUSTED_CONTENT_MARKER).unwrap();
        let hostile_at = result
            .text
            .rfind(hostile_text)
            .expect("the hostile text is in the record");
        assert!(
            hostile_at > framing_ends_at,
            "the hostile text sits after the warning, inside the data: {}",
            result.text
        );
    }
}
...
#[test]
fn a_server_chosen_bidi_filename_is_sanitized_before_mcp_sees_it() {
    let mut mcp = McpRig::start();
    mcp.server.serve(
        "/bidi",
        Resource {
            // Header field values are ASCII here; RFC 8187 is the normal way
            // a remote server sends this Unicode filename.
            content_disposition: Some(
                "attachment; filename*=UTF-8''invoice%E2%80%AEfdp.exe".to_string(),
            ),
            ..Resource::new(body(1_000))
        },
    );

    let started = mcp.call("download_file", json!({"url": mcp.url("/bidi")}));
    let id = started.json()["transfer"]["id"].as_u64().unwrap();
    let done = mcp.wait_for_state(id, "completed");
    let listed = mcp.call("list_transfers", json!({}));

    for result in [&done, &listed] {
        assert!(
            result.text.contains(UNTRUSTED_CONTENT_MARKER),
            "{}",
            result.text
        ...
        assert!(
            ...
            "a bidi override must never reach an MCP record: {}",
            result.text
        );
        assert!(
            result.text.contains("invoicefdp.exe"),
            "the visible filename is the sanitized one: {}",
            result.text
        );
    }
    let dest = PathBuf::from(done.json()["transfer"]["dest_path"].as_str().unwrap());
    assert!(dest.ends_with("invoicefdp.exe"));
    assert_eq!(std::fs::read(dest).unwrap(), body(1_000));
}
