...
//! Kabuverdianu raw CLDR NumberFormat unit-pattern regressions.

use super::*;

#[test]
fn sources_kabuverdianu_cldr_units_per_patterns_and_generic_compounds() {
    let formatter = |unit, display| {
        NumberFormat::try_new(
            &[canonicalize("kea").unwrap()],
            NumberFormatOptions {
