...
pub(crate) fn cldr_vietnamese_per_unit_pattern(
    locale: &str,
...
pub(crate) fn cldr_vietnamese_generic_compound_unit_pattern(
    locale: &str,
    ...
    let numerator_raw = cldr_vietnamese_additional_unit_pattern(locale, numerator, display, plural);
    let denominator_display = match display {
        Display::Long => Display::Long,
        Display::Short | Display::Narrow => Display::Narrow,
    };
    let denominator_raw = cldr_vietnamese_additional_unit_pattern(
        locale,
        denominator_unit,
        denominator_display,
    ...
    let denominator = denominator_raw
        ...
        // Retain the locale-aware duration label when the pinned typed marker
        // lacks Vietnamese data, rather than losing the raw numerator too.
        .unwrap_or_else(|| {
