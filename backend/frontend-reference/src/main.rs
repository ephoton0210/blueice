...
#[cfg(not(unix))]
fn main() {
    eprintln!("blueice-frontend is currently supported only on Unix platforms");
    std::process::exit(1);
}
...
mod unix {
    ...
    impl ApplicationHandler<UserEvent> for App {
        fn resumed(&mut self, event_loop: &ActiveEventLoop) {
            ...
            let title =
                blueice_i18n::translate(self.locale, "frontend", "window-title-default", &[]);
    ...
    pub(super) fn main() {
        ...
        let mut writer =
            UnixStream::connect(&socket_path).expect("failed to connect to blueice-core");
    ...
    mod tests {
        ...
        fn wait_for_socket_returns_true_once_the_path_exists() {
            let path =
                std::env::temp_dir().join(format!("blueice-wait-test-{}", std::process::id()));
