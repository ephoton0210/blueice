...
use blueice_ipc::downloads::{default_downloads_socket_path, format_bytes, format_duration, format_speed, DownloadsClient, SegmentInfo, SingleStreamReason, TransferInfo, TransferMode, TransferState};
use std::io;
...
fn sorted(transfers: &[TransferInfo]) -> Vec<&TransferInfo> {
    let running = |t: &TransferInfo| matches!(t.state, TransferState::Active | TransferState::AwaitingClearance | TransferState::Queued);
...
fn display_name(info: &TransferInfo) -> String {
    ...
    match from_dest {
        ...
        None => blueice_net::download::file_name::file_name_from_url(&info.url).unwrap_or_else(|| info.url.clone()),
    }
}
...
pub fn downloads_html(view: &DownloadsView<'_>, locale: &str) -> String {
    let locale = if blueice_i18n::SUPPORTED_LOCALES.contains(&locale) { locale } else { blueice_i18n::DEFAULT_LOCALE };
    let t = |key: &str| blueice_i18n::translate(locale, "downloads", key, &[]);

    let mut body = format!("<h1>{}</h1>", escape_html(&t("downloads-title")));
    match view {
        DownloadsView::Unavailable => body.push_str(&format!("<p class=\"error\">{}</p>", escape_html(&t("downloads-unavailable")))),
        DownloadsView::Transfers([]) => body.push_str(&format!("<p class=\"summary\">{}</p>", escape_html(&t("downloads-empty")))),
        DownloadsView::Transfers(transfers) => {
            let count = if transfers.len() == 1 { t("downloads-count-one") } else { format!("{} {}", transfers.len(), t("downloads-count-other")) };
    ...
    format!("<html><head><title>{}</title><style>{STYLE}</style></head><body>{body}</body></html>", escape_html(&t("downloads-title")))
}
...
fn push_transfer(body: &mut String, info: &TransferInfo, t: &dyn Fn(&str) -> String) {
    ...
    let progress = match info.total_bytes {
        Some(total) => format!("{pct}% ({} {} {})", format_bytes(info.completed_bytes), t("label-of"), format_bytes(total)),
        None => format!("{} ({})", format_bytes(info.completed_bytes), t("label-total-unknown")),
    };
    let class = format!("transfer state-{}", info.state.as_str().replace('_', "-"));
    body.push_str(&format!("<div class=\"{class}\"><p class=\"name\">{}</p>", escape_html(&display_name(info))));
    ...
    let status = if info.completed_bytes > 0 || info.total_bytes.is_some() { format!("{} \u{2014} {progress}", state_label(info.state, t)) } else { state_label(info.state, t) };
    body.push_str(&format!("<p class=\"status\">{}</p>", escape_html(&status)));
    body.push_str(&format!("<div class=\"bar\"><div class=\"fill\" style=\"width: {pct}%\"></div></div>"));

    let mut live: Vec<String> = Vec::new();
    if info.speed_bps > 0 {
        live.push(format!("{} {}", t("label-speed"), format_speed(info.speed_bps)));
    }
    ...
    if !live.is_empty() {
        body.push_str(&format!("<p class=\"detail\">{}</p>", escape_html(&live.join(" \u{b7} "))));
    }

    let mode = match info.mode {
        TransferMode::Segmented => Some(t("mode-segmented")),
        TransferMode::SingleStream { reason: SingleStreamReason::UnknownLength } => Some(t("mode-single-unknown-length")),
        TransferMode::SingleStream { .. } => Some(t("mode-single-stream")),
        TransferMode::Undetermined | TransferMode::Unknown => None,
    };
    if let Some(mode) = mode {
        body.push_str(&format!("<p class=\"detail\">{} {}</p>", escape_html(&t("label-mode")), escape_html(&mode)));
    }
    if info.retries > 0 {
        body.push_str(&format!("<p class=\"detail\">{} {}</p>", escape_html(&t("label-retries")), info.retries));
    }
    if let Some(error) = &info.last_error {
        body.push_str(&format!("<p class=\"detail error\">{} {}</p>", escape_html(&t("label-last-error")), escape_html(error)));
    }
    if let Some(blocked) = &info.blocked {
        body.push_str(&format!("<p class=\"detail error\">{} {} ({})</p>", escape_html(&t("label-blocked")), escape_html(&blocked.reason), escape_html(&blocked.category)));
    }
    if info.state == TransferState::Completed {
        body.push_str(&format!("<p class=\"detail\">{} {}</p>", escape_html(&t("label-saved-to")), escape_html(&info.dest_path)));
    }
    if info.state == TransferState::Paused && !info.resume_safe {
        body.push_str(&format!("<p class=\"detail\">{}</p>", escape_html(&t("note-resume-unsafe"))));
    }
    if info.segments.len() >= 2 && matches!(info.state, TransferState::Active | TransferState::Paused | TransferState::Failed) {
        body.push_str(&format!("<p class=\"detail\">{}</p>", escape_html(&t("segments-heading"))));
        for segment in info.segments.iter().take(MAX_SEGMENT_BARS) {
            body.push_str(&format!("<div class=\"seg\"><div class=\"segfill\" style=\"width: {}%\"></div></div>", segment_percent(segment)));
        }
    }
    body.push_str("</div>");
}
...
fn spawn_sibling_downloads_process(socket: &Path) -> io::Result<()> {
    ...
    let mut child = Command::new(dir.join("blueice-downloads")).arg("--socket").arg(socket).stdin(Stdio::null()).stdout(Stdio::null()).stderr(Stdio::null()).spawn()?;
    thread::spawn(move || {
        let _ = child.wait();
    });
    Ok(())
...
impl DownloadsSource {
    ...
    pub fn at(socket: PathBuf) -> Self {
        ...
        DownloadsSource::with_spawner(socket, Box::new(move || spawn_sibling_downloads_process(&for_spawn)), SPAWN_TIMEOUT)
    }
    ...
    pub fn without_spawner(socket: PathBuf) -> Self {
        DownloadsSource { socket, spawner: None, timeout: SPAWN_TIMEOUT }
    }

    pub fn with_spawner(socket: PathBuf, spawner: Spawner, timeout: Duration) -> Self {
        DownloadsSource { socket, spawner: Some(spawner), timeout }
    }
    ...
    fn fetch_within(&self, spawn: bool, limit: Duration) -> Result<Vec<TransferInfo>, String> {
        ...
        let stream = match UnixStream::connect(&self.socket) {
            ...
            Err(_) => {
                let Some(spawner) = &self.spawner else { return Err("the downloads service is not running and cannot be started from here".to_string()) };
                spawner().map_err(|e| format!("the downloads service could not be started: {e}"))?;
                loop {
                    match UnixStream::connect(&self.socket) {
                        Ok(stream) => break stream,
                        Err(_) if Instant::now() < deadline => thread::sleep(Duration::from_millis(20)),
                        Err(_) => return Err("the downloads service was started but did not start listening in time".to_string()),
                    }
                }
            }
        };
        ...
        // On a helper thread, so the deadline holds even against a peer that
        // sends half a frame and stalls (the framing layer retries a read
        // timeout once some bytes have arrived).
        let (tx, rx) = mpsc::channel();
        thread::spawn(move || {
            let result = DownloadsClient::connect(stream).and_then(|mut client| client.list(None)).map_err(|e| e.to_string());
            let _ = tx.send(result);
        });
        rx.recv_timeout(remaining).map_err(|_| "the downloads service did not answer in time".to_string())?
    }
}
...
pub(crate) mod test_support {
    use blueice_ipc::downloads::{read_downloads_request, write_downloads_reply, DownloadsReply, DownloadsRequest, TransferInfo};
    ...
    impl Scratch {
        pub(crate) fn new(label: &str) -> Self {
            ...
            let path = std::env::temp_dir().join(format!("be-dl-{label}-{}-{}", std::process::id(), NEXT.fetch_add(1, Ordering::Relaxed)));
    ...
    pub(crate) fn fake_downloads(socket: &std::path::Path, transfers: Vec<TransferInfo>, stall: bool, version: u32) -> thread::JoinHandle<()> {
        fake_downloads_live(socket, FakeState { transfers: Arc::new(Mutex::new(transfers)), lists: Arc::default() }, stall, version)
    }

    pub(crate) fn fake_downloads_live(socket: &std::path::Path, state: FakeState, stall: bool, version: u32) -> thread::JoinHandle<()> {
                ...
                thread::spawn(move || {
                    while let Ok((id, request)) = read_downloads_request(&mut stream) {
                        ...
                        let reply = match request {
                            DownloadsRequest::Hello { .. } => DownloadsReply::Hello { protocol_version: version },
...
mod tests {
    ...
    fn the_downloads_url_uses_the_about_scheme_and_may_carry_a_language() {
        ...
        for other in ["about:downloadsx", "about:credits", "about:blank", "https://example.com/about:downloads", "downloads"] {
    ...
    fn an_empty_list_says_so_and_shows_no_transfer_rows() {
        let html = en(&[]);
        assert!(html.contains("<title>Downloads</title>") && html.contains("<h1>Downloads</h1>"), "{html}");
    ...
    fn an_unreachable_service_is_reported_instead_of_an_empty_list() {
        ...
        assert!(html.contains("The downloads service is not running"), "{html}");
        assert!(!html.contains("No downloads yet."), "an unreachable service is not the same as having no downloads");
    }
    ...
    fn a_running_transfer_shows_name_status_progress_bar_and_live_figures() {
        ...
        let html = en(&[t]);
        assert!(html.contains("file1.iso"), "the name is the destination's file name: {html}");
        assert!(html.contains("Downloading"), "{html}");
        assert!(html.contains("40% (1.2 MiB of 3.0 MiB)"), "{html}");
        assert!(html.contains("style=\"width: 40%\""), "the bar's fill is 40% wide: {html}");
        assert!(html.contains("Speed:") && html.contains("12.3 MiB/s"), "{html}");
        assert!(html.contains("Time left:") && html.contains("1 min 20 s"), "{html}");
        assert!(html.contains("Connections:") && html.contains('8'), "{html}");
    ...
    fn each_connection_gets_its_own_progress_bar_when_there_are_several() {
        let mut t = transfer(1, TransferState::Active);
        t.segments = vec![
            SegmentInfo { start: 0, end: 1_000, completed: 750, state: SegmentState::Active },
            SegmentInfo { start: 1_000, end: 2_000, completed: 1_000, state: SegmentState::Done },
            SegmentInfo { start: 2_000, end: 3_000, completed: 0, state: SegmentState::Retrying },
        ];
        let html = en(&[t]);
        assert!(html.contains("Progress of each connection"), "{html}");
        for width in ["75%", "100%", "0%"] {
            assert!(html.contains(&format!("class=\"segfill\" style=\"width: {width}\"")), "segment fill {width} in {html}");
        }
        // a single segment is just the main bar again -- no redundant list
        let mut one = transfer(2, TransferState::Active);
        one.segments = vec![SegmentInfo { start: 0, end: 10, completed: 5, state: SegmentState::Active }];
    ...
    fn a_zero_length_segment_does_not_divide_by_zero() {
        let mut t = transfer(1, TransferState::Active);
        t.segments = vec![SegmentInfo { start: 5, end: 5, completed: 0, state: SegmentState::Pending }, SegmentInfo { start: 5, end: 10, completed: 1, state: SegmentState::Active }];
    ...
    fn a_completed_transfer_is_a_full_bar_and_says_where_the_file_is() {
        ...
        let html = en(&[t]);
        assert!(html.contains("Completed") && html.contains("100%") && html.contains("style=\"width: 100%\""), "{html}");
        assert!(html.contains("Saved to:") && html.contains("/home/u/Downloads/BlueIce/file1.iso"), "{html}");
    }
    ...
    fn a_failed_transfer_shows_its_last_error() {
        ...
        let html = en(&[t]);
        assert!(html.contains("Failed") && html.contains("Last error:") && html.contains("HTTP status 404"), "{html}");
    }
    ...
    fn a_blocked_transfer_says_the_gatekeeper_stopped_it_and_why() {
        ...
        let html = en(&[t]);
        assert!(html.contains("Blocked") && html.contains("Blocked by the safety gatekeeper:"), "{html}");
        assert!(html.contains("dangerous-file-type") && html.contains("executable from an untrusted origin"), "{html}");
    }
    ...
    fn retries_and_a_single_stream_mode_are_explained() {
        ...
        assert!(html.contains("the server does not support parallel downloads"), "{html}");
        let mut u = transfer(2, TransferState::Active);
        u.mode = TransferMode::SingleStream { reason: SingleStreamReason::UnknownLength };
    ...
    fn a_transfer_that_has_moved_nothing_and_knows_no_size_shows_just_its_state() {
        for state in [TransferState::Queued, TransferState::AwaitingClearance, TransferState::Failed, TransferState::Blocked, TransferState::Cancelled] {
            let mut t = transfer(1, state);
            t.completed_bytes = 0;
            t.total_bytes = None;
            let html = en(&[t]);
            assert!(!html.contains("0 B") && !html.contains("total size unknown"), "{state}: {html}");
        }
    ...
    fn a_queued_transfer_has_no_speed_or_progress_figures_to_show() {
        ...
        assert!(!html.contains("Speed:") && !html.contains("Time left:"), "{html}");
    }
    ...
    fn every_state_has_its_own_label() {
        for (state, label) in [
            ...
            (TransferState::AwaitingClearance, "Waiting for safety review"),
            ...
            assert!(en(&[transfer(1, state)]).contains(label), "{state}: {label}");
        }
    }

    #[test]
    fn the_count_is_singular_or_plural() {
        assert!(en(&[transfer(1, TransferState::Active)]).contains("1 download<"), "{}", en(&[transfer(1, TransferState::Active)]));
        assert!(en(&[transfer(1, TransferState::Active), transfer(2, TransferState::Paused), transfer(3, TransferState::Failed)]).contains("3 downloads"));
    }

    #[test]
    fn running_transfers_come_first_then_the_rest_newest_first() {
        let html = en(&[transfer(1, TransferState::Completed), transfer(2, TransferState::Active), transfer(3, TransferState::Failed), transfer(4, TransferState::Paused), transfer(5, TransferState::Queued)]);
        ...
        assert!(at("file5.iso") < at("file2.iso"), "queued 5 before active 2 (both running, newest first)");
        assert!(at("file2.iso") < at("file4.iso"), "running before paused");
        assert!(at("file4.iso") < at("file3.iso") && at("file3.iso") < at("file1.iso"), "the rest newest first");
    }
    ...
    fn text_a_remote_server_controls_cannot_inject_markup() {
        ...
        t.blocked = Some(BlockedInfo { reason: "<img src=x onerror=y>".to_string(), category: "a&b".to_string() });
        let html = en(&[t]);
        assert!(!html.contains("<script>") && !html.contains("<img") && !html.contains("<b>bold"), "{html}");
        assert!(html.contains("&lt;script&gt;") && html.contains("&lt;b&gt;bold&lt;/b&gt; &amp; &quot;quoted&quot; &#39;single&#39;"), "{html}");
    ...
    fn the_page_name_falls_back_to_the_url_when_there_is_no_destination_yet() {
        ...
        assert!(en(&[t]).contains("https://example.com/"), "with nothing better, the whole URL");
    }

    #[test]
    fn the_page_is_localized_through_i18n() {
        let html = downloads_html(&DownloadsView::Transfers(&[transfer(1, TransferState::Active), transfer(2, TransferState::Blocked)]), "zh-TW");
        assert!(html.contains("<title>下載</title>"), "{html}");
        assert!(html.contains("下載中") && html.contains("已封鎖"), "{html}");
        assert!(!html.contains("Downloading"), "no English state label leaks into a translated page: {html}");
        assert!(downloads_html(&DownloadsView::Unavailable, "zh-TW").contains("下載服務尚未執行"));
        assert!(downloads_html(&DownloadsView::Transfers(&[]), "zh-TW").contains("目前沒有任何下載"));
    }

    #[test]
    fn an_unsupported_locale_falls_back_to_english() {
        assert!(downloads_html(&DownloadsView::Transfers(&[]), "klingon").contains("No downloads yet."));
    }
    ...
    fn a_running_downloads_service_is_read_for_its_list() {
        ...
        let _server = fake_downloads(&dir.socket(), vec![transfer(1, TransferState::Active), transfer(2, TransferState::Completed)], false, DOWNLOADS_PROTOCOL_VERSION);
    ...
    fn a_missing_service_is_an_error_and_is_not_started_by_the_quick_read() {
        ...
        let source = DownloadsSource::with_spawner(dir.socket(), Box::new(move || { counter.fetch_add(1, Ordering::SeqCst); Ok(()) }), Duration::from_millis(300));
        assert!(source.fetch_quick().is_err());
        assert_eq!(spawned.load(Ordering::SeqCst), 0, "merely rendering the page quickly must not start a process");
    }
    ...
    fn opening_the_page_starts_the_service_when_it_is_not_running() {
        ...
        let spawner: Spawner = Box::new(move || {
            ...
            thread::spawn(move || {
                ...
                let _ = fake_downloads(&socket, vec![transfer(7, TransferState::Paused)], false, DOWNLOADS_PROTOCOL_VERSION);
            });
            Ok(())
        });
    ...
    fn a_spawn_that_fails_or_never_yields_a_socket_is_an_error_not_a_hang() {
        ...
        let failing = DownloadsSource::with_spawner(dir.socket(), Box::new(|| Err(std::io::Error::other("no such binary"))), Duration::from_secs(1));
        assert!(failing.fetch_spawning().unwrap_err().contains("no such binary"));
        let silent = DownloadsSource::with_spawner(dir.socket(), Box::new(|| Ok(())), Duration::from_millis(300));
    ...
    fn a_service_that_never_answers_is_given_up_on_at_the_timeout() {
        ...
        let source = DownloadsSource::with_spawner(dir.socket(), Box::new(|| Ok(())), Duration::from_millis(300));
        let started = Instant::now();
        let error = source.fetch_quick().unwrap_err();
        assert!(started.elapsed() < Duration::from_secs(3), "must not stall the caller, took {:?}", started.elapsed());
    ...
    fn a_service_speaking_another_protocol_version_is_an_error() {
        ...
        let _server = fake_downloads(&dir.socket(), Vec::new(), false, DOWNLOADS_PROTOCOL_VERSION + 1);
        assert!(DownloadsSource::without_spawner(dir.socket()).fetch_quick().is_err());
    }

    #[test]
    fn the_production_source_points_at_the_well_known_socket() {
        let source = DownloadsSource::new();
        assert_eq!(source.socket(), blueice_ipc::downloads::default_downloads_socket_path());
    }
}
