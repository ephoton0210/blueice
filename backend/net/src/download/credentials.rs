...
impl SftpCredentialRef {
    pub fn new(
        ...
        if host.is_empty() || host.chars().any(char::is_control) {
            return Err(CredentialError::InvalidReference(
                "SFTP host must be non-empty and contain no control characters".to_string(),
            ));
        }
        if port == 0 {
            return Err(CredentialError::InvalidReference(
                "SFTP port must be between 1 and 65535".to_string(),
            ));
        }
        if username.is_empty() || username.chars().any(char::is_control) {
            return Err(CredentialError::InvalidReference(
                "SFTP username must be non-empty and contain no control characters".to_string(),
            ));
        }
        Ok(SftpCredentialRef {
            host,
            port,
            username,
        })
    ...
    fn entry(&self) -> Result<Entry, CredentialError> {
        ...
        Entry::new(
            &format!("{SFTP_SERVICE_PREFIX}.{}:{}", self.host, self.port),
            &self.username,
        )
...
impl SftpPrivateKeyPassphraseRef {
    pub fn new(
        ...
        Ok(SftpPrivateKeyPassphraseRef {
            host,
            port,
            username,
        })
    ...
    fn entry(&self) -> Result<Entry, CredentialError> {
        Entry::new(
            &format!(
                "{SFTP_KEY_PASSPHRASE_SERVICE_PREFIX}.{}:{}",
                self.host, self.port
            ),
            &self.username,
        )
...
impl FtpsCredentialRef {
    pub fn new(
        ...
        if host.is_empty() || host.chars().any(char::is_control) {
            return Err(CredentialError::InvalidReference(
                "FTPS host must be non-empty and contain no control characters".to_string(),
            ));
        }
        if port == 0 {
            return Err(CredentialError::InvalidReference(
                "FTPS port must be between 1 and 65535".to_string(),
            ));
        }
        if username.is_empty() || username.chars().any(char::is_control) {
            return Err(CredentialError::InvalidReference(
                "FTPS username must be non-empty and contain no control characters".to_string(),
            ));
        }
        Ok(FtpsCredentialRef {
            host,
            port,
            username,
        })
    ...
    fn entry(&self) -> Result<Entry, CredentialError> {
        Entry::new(
            &format!("{FTPS_SERVICE_PREFIX}.{}:{}", self.host, self.port),
            &self.username,
        )
        .map_err(|_| CredentialError::Unavailable)
    }
    ...
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        match self {
            ...
            CredentialError::Unavailable => {
                f.write_str("the operating system credential store is unavailable")
            }
        }
    }
}
...
pub fn save_sftp_password(
    reference: &SftpCredentialRef,
    password: &str,
) -> Result<(), CredentialError> {
    if password.is_empty() {
        return Err(CredentialError::InvalidReference(
            "SFTP password must not be empty".to_string(),
        ));
    }
    reference
        .entry()?
...
pub fn save_sftp_private_key_passphrase(
    reference: &SftpPrivateKeyPassphraseRef,
    passphrase: &str,
) -> Result<(), CredentialError> {
    if passphrase.is_empty() {
        return Err(CredentialError::InvalidReference(
            "SFTP private-key passphrase must not be empty".to_string(),
        ));
    }
    reference
        .entry()?
...
pub fn delete_sftp_private_key_passphrase(
    reference: &SftpPrivateKeyPassphraseRef,
) -> Result<(), CredentialError> {
    match reference.entry()?.delete_credential() {
        Ok(()) | Err(keyring::Error::NoEntry) => Ok(()),
...
pub fn save_ftps_password(
    reference: &FtpsCredentialRef,
    password: &str,
) -> Result<(), CredentialError> {
    if password.is_empty() {
        return Err(CredentialError::InvalidReference(
            "FTPS password must not be empty".to_string(),
        ));
    }
    reference
        .entry()?
...
mod tests {
    ...
    fn a_private_key_passphrase_reference_has_its_own_credential_namespace() {
        let reference =
            SftpPrivateKeyPassphraseRef::new("files.example.test", 2222, "alice").unwrap();
        assert_eq!(reference.host, "files.example.test");
        assert_eq!(reference.port, 2222);
        assert_eq!(reference.username, "alice");
        assert!(matches!(
            SftpPrivateKeyPassphraseRef::new("host", 22, "alice\nroot"),
    ...
    fn ftp_and_sftp_references_use_distinct_protocol_namespaces() {
        ...
        assert!(matches!(
            FtpsCredentialRef::new("host", 0, "alice"),
            Err(CredentialError::InvalidReference(_))
        ));
    }
    ...
    fn an_empty_or_control_character_identity_is_rejected() {
        assert!(matches!(
            SftpCredentialRef::new("", 22, "alice"),
            Err(CredentialError::InvalidReference(_))
        ));
        assert!(matches!(
            SftpCredentialRef::new("host", 0, "alice"),
            Err(CredentialError::InvalidReference(_))
        ));
        assert!(matches!(
            SftpCredentialRef::new("host", 22, "alice\nroot"),
            Err(CredentialError::InvalidReference(_))
        ));
    }
}
