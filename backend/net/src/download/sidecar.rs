...
impl Sidecar {
    ...
    pub fn check(&self, probe: &Probe, data_len: u64) -> Result<(), RestartReason> {
        ...
        match (stored_validator(&self.etag, &self.last_modified), probe.validator()) {
    ...
    fn check_segments(&self) -> Result<(), RestartReason> {
        ...
        for segment in &segments {
            if segment.start >= segment.end || segment.pos < segment.start || segment.pos > segment.end {
                return Err(RestartReason::InconsistentSegments);
            }
        }
    ...
    pub fn load(dest: &Path) -> Option<Sidecar> {
        serde_json::from_slice(&std::fs::read(sidecar_path(dest)).ok()?).ok()
    }
}
...
pub fn remove_partials(dest: &Path) {
    ...
    let _ = std::fs::remove_file(append_suffix(&sidecar, ".tmp"));
...
mod tests {
    ...
    fn segments() -> Vec<SidecarSegment> {
        vec![SidecarSegment { start: 0, end: 500, pos: 200 }, SidecarSegment { start: 500, end: 1_000, pos: 500 }]
    }
    ...
    fn the_part_file_and_sidecar_sit_next_to_the_destination() {
        assert_eq!(part_path(Path::new("/d/file.iso")), Path::new("/d/file.iso.blueice-part"));
        assert_eq!(sidecar_path(Path::new("/d/file.iso")), Path::new("/d/file.iso.blueice-part.json"));
        assert_eq!(part_path(Path::new("/d/noext")), Path::new("/d/noext.blueice-part"));
        assert_eq!(sidecar_path(Path::new("rel/a.tar.gz")), Path::new("rel/a.tar.gz.blueice-part.json"));
    }
    ...
    fn segments_need_not_be_stored_in_order() {
        ...
        segs.reverse();
        assert_eq!(Sidecar::from_probe(&probe(), segs).check(&probe(), 1_000), Ok(()));
    }
    ...
    fn an_unsupported_version_is_rejected() {
        ...
        assert_eq!(s.check(&probe(), 1_000), Err(RestartReason::UnsupportedVersion));
    }

    #[test]
    fn a_different_url_is_rejected() {
        let other = Probe { url: "http://h/other".to_string(), ..probe() };
        assert_eq!(valid().check(&other, 1_000), Err(RestartReason::DifferentUrl));
    }

    #[test]
    fn a_different_total_size_is_rejected() {
        let other = Probe { total: Some(2_000), ..probe() };
        assert_eq!(valid().check(&other, 1_000), Err(RestartReason::DifferentSize));
        let unknown = Probe { total: None, ..probe() };
        assert_eq!(valid().check(&unknown, 1_000), Err(RestartReason::DifferentSize));
    }

    #[test]
    fn resuming_without_anything_to_validate_against_is_refused() {
        // The fresh probe carries no validator...
        let bare = Probe { etag: None, last_modified: None, ..probe() };
        ...
        assert_eq!(stored.check(&probe(), 1_000), Err(RestartReason::NoValidator));
    }

    #[test]
    fn a_changed_etag_or_last_modified_is_rejected() {
        let changed_etag = Probe { etag: Some("\"different\"".to_string()), ..probe() };
        assert_eq!(valid().check(&changed_etag, 1_000), Err(RestartReason::ValidatorChanged));

        let only_lm = Probe { etag: None, ..probe() };
        let stored = Sidecar::from_probe(&only_lm, segments());
        let changed_lm = Probe { last_modified: Some("Thu, 22 Oct 2015 07:28:00 GMT".to_string()), ..only_lm.clone() };
        assert_eq!(stored.check(&changed_lm, 1_000), Err(RestartReason::ValidatorChanged));
    }
    ...
    fn a_validator_of_a_different_kind_is_conservatively_a_change() {
        ...
        let only_lm = Probe { etag: None, ..probe() };
        let stored = Sidecar::from_probe(&only_lm, segments());
        assert_eq!(stored.check(&probe(), 1_000), Err(RestartReason::ValidatorChanged));
    }

    #[test]
    fn a_data_file_of_the_wrong_length_is_rejected() {
        assert_eq!(valid().check(&probe(), 999), Err(RestartReason::DataFileWrongLength));
        assert_eq!(valid().check(&probe(), 0), Err(RestartReason::DataFileWrongLength));
    }
    ...
    fn inconsistent_segment_lists_are_rejected() {
        let cases: Vec<(&str, Vec<SidecarSegment>)> = vec![
            ("empty", vec![]),
            ("does not start at 0", vec![SidecarSegment { start: 1, end: 1_000, pos: 1 }]),
            ("does not reach the total", vec![SidecarSegment { start: 0, end: 999, pos: 0 }]),
            ("gap", vec![SidecarSegment { start: 0, end: 400, pos: 0 }, SidecarSegment { start: 500, end: 1_000, pos: 500 }]),
            ("overlap", vec![SidecarSegment { start: 0, end: 600, pos: 0 }, SidecarSegment { start: 500, end: 1_000, pos: 500 }]),
            ("pos before start", vec![SidecarSegment { start: 0, end: 500, pos: 0 }, SidecarSegment { start: 500, end: 1_000, pos: 499 }]),
            ("pos after end", vec![SidecarSegment { start: 0, end: 500, pos: 501 }, SidecarSegment { start: 500, end: 1_000, pos: 500 }]),
            ("empty span", vec![SidecarSegment { start: 0, end: 0, pos: 0 }, SidecarSegment { start: 0, end: 1_000, pos: 0 }]),
        ];
        for (what, segs) in cases {
            assert_eq!(Sidecar::from_probe(&probe(), segs).check(&probe(), 1_000), Err(RestartReason::InconsistentSegments), "{what}");
        }
    }
    ...
    fn saving_is_atomic_and_leaves_no_temporary_file() {
        ...
        assert_eq!(Sidecar::load(&dest).unwrap().segments[0].pos, 300, "a second save replaces the first");
    ...
    fn a_missing_or_corrupt_sidecar_loads_as_none() {
        ...
        assert_eq!(Sidecar::load(&dest), None, "a JSON object without the required fields is corrupt too");
    }
    ...
    fn a_sidecar_from_another_version_still_loads_so_check_can_name_the_reason() {
        ...
        s.version = 99;
        s.save(&dest).unwrap();
        assert_eq!(Sidecar::load(&dest).unwrap().check(&probe(), 1_000), Err(RestartReason::UnsupportedVersion));
    }

    #[test]
    fn saving_into_a_missing_directory_is_an_error() {
        let dir = TempDir::new("sidecar-missing-dir");
        assert!(valid().save(&dir.path().join("no-such-dir").join("file.iso")).is_err());
    }
