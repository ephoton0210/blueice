...
impl Rig {
    fn new() -> Self {
        Rig { server: TestServer::start(), gate: FakeGatekeeper::clear_all(), dir: TempDir::new() }
    }
    ...
    fn review(&self, path: &str, dest: &Path, options: &DownloadOptions) -> (Probe, DownloadClearance) {
    ...
    fn try_begin(&self, path: &str, dest: &Path, options: DownloadOptions) -> Result<Transfer, DownloadError> {
        let (probed, clearance) = self.review(path, dest, &options);
        Transfer::begin(DownloadSpec { dest: dest.to_path_buf(), options, on_update: None }, probed, clearance)
    }
...
fn finish(transfer: &Transfer) -> Snapshot {
    transfer.wait_timeout(Duration::from_secs(30)).expect("the transfer did not settle in time")
}
...
fn slow_resource(len: usize, delay_ms: u64) -> Resource {
    Resource { chunk: 16 * KIB, delay_per_chunk: Duration::from_millis(delay_ms), ..Resource::new(body(len)) }
}
...
fn a_file_is_downloaded_in_parallel_segments_and_arrives_byte_for_byte() {
    ...
    assert!(done.segments.iter().all(|s| s.state == SegmentState::Done && s.completed == s.end - s.start));
    assert!(rig.server.peak_concurrency() >= 2, "the segments must have been fetched concurrently, peak was {}", rig.server.peak_concurrency());
...
fn segments_cover_the_file_with_ranged_requests_that_carry_the_validator() {
    ...
    assert!(segment_requests.iter().all(|r| r.if_range.as_deref() == Some("\"v1\"")), "{segment_requests:?}");
    assert!(segment_requests.iter().all(|r| r.accept_encoding.as_deref() == Some("identity")));
    ...
    for (start, end) in &ranges {
        assert!(*start <= covered_to, "a gap before byte {start}: covered only to {covered_to} in {ranges:?}");
...
fn an_idle_connection_splits_the_slowest_segment_instead_of_finishing_early() {
    ...
    let slow: Arc<dyn Fn(Option<u64>) -> Duration + Send + Sync> = Arc::new(|start| if start == Some(0) { Duration::from_millis(25) } else { Duration::ZERO });
    rig.server.serve("/f.bin", Resource { chunk: 16 * KIB, slow: Some(slow), ..Resource::new(body(MIB)) });
    let dest = rig.dest("f.bin");
    let options = DownloadOptions { max_connections: 2, min_split_bytes: 32 * KIB as u64, ..Rig::options() };

    let transfer = rig.begin("/f.bin", &dest, options);
    let done = finish(&transfer);

    assert_eq!(done.state, TransferState::Completed);
    assert_eq!(std::fs::read(&dest).unwrap(), body(MIB), "splitting must not corrupt the file");
    let first_half = 0..(MIB as u64 / 2);
    let split_inside_slow_half = ranged_requests(&rig.server).into_iter().skip(1).any(|(start, _)| start > 0 && first_half.contains(&start));
    assert!(split_inside_slow_half, "expected a request starting inside the slow first half: {:?}", ranged_requests(&rig.server));
...
fn a_server_that_ignores_ranges_is_downloaded_as_one_plain_stream() {
    let rig = Rig::new();
    rig.server.serve("/f.bin", Resource { honor_ranges: false, ..Resource::new(body(300 * KIB)) });
    let dest = rig.dest("f.bin");
    let transfer = rig.begin("/f.bin", &dest, Rig::options());
    let snapshot = transfer.snapshot();
    assert_eq!(snapshot.mode, TransferMode::SingleStream { reason: SingleStreamReason::ServerIgnoresRange });
    let done = finish(&transfer);

    assert_eq!(done.state, TransferState::Completed);
    assert_eq!(std::fs::read(&dest).unwrap(), body(300 * KIB));
    let log = rig.server.requests();
    assert_eq!(log.len(), 2, "the probe plus exactly one transfer request: {log:?}");
...
fn a_response_of_unknown_length_completes_when_the_stream_ends() {
    let rig = Rig::new();
    rig.server.serve("/stream", Resource { honor_ranges: false, send_content_length: false, ..Resource::new(body(200 * KIB)) });
    let dest = rig.dest("stream.bin");
    let transfer = rig.begin("/stream", &dest, Rig::options());
    assert_eq!(transfer.snapshot().mode, TransferMode::SingleStream { reason: SingleStreamReason::ServerIgnoresRange });
    let done = finish(&transfer);
    assert_eq!(done.state, TransferState::Completed);
...
fn an_empty_file_completes_at_once_as_an_empty_destination() {
    ...
    assert_eq!(rig.server.requests().len(), 1, "nothing to fetch beyond the probe");
}
...
fn transient_server_errors_are_retried_with_backoff_and_the_download_still_completes() {
    ...
    rig.server.update("/f.bin", |r| r.fail_statuses = vec![503, 503]);

    let transfer = Transfer::begin(DownloadSpec { dest: dest.clone(), options, on_update: None }, probed, clearance).unwrap();
    let done = finish(&transfer);
...
fn a_connection_cut_mid_body_is_retried_from_where_it_stopped() {
    ...
    let starts: Vec<u64> = ranged_requests(&rig.server).into_iter().skip(1).map(|(s, _)| s).collect();
    assert!(starts.iter().any(|s| s % (MIB as u64 / 4) != 0), "every request began on a segment boundary, so a cut segment was refetched from its start: {starts:?}");
}
...
fn progress_between_failures_resets_the_retry_budget() {
    ...
    let dest = rig.dest("f.bin");
    let options = DownloadOptions { max_retries: 1, ..Rig::options() };
    let (probed, clearance) = rig.review("/f.bin", &dest, &options);
    rig.server.update("/f.bin", |r| {
    ...
    let done = finish(&Transfer::begin(DownloadSpec { dest: dest.clone(), options, on_update: None }, probed, clearance).unwrap());
    assert_eq!(done.state, TransferState::Completed, "{done:?}");
    assert_eq!(done.segments.len(), 1);
    assert_eq!(done.retries, 3, "three cuts, three retries, none of them exhausting a budget of one");
...
fn too_many_consecutive_failures_fail_the_transfer_but_keep_the_partial_download() {
    ...
    rig.server.serve("/f.bin", Resource::new(body(MIB)));
    let dest = rig.dest("f.bin");
    let options = DownloadOptions { max_retries: 2, ..Rig::options() };
    let (probed, clearance) = rig.review("/f.bin", &dest, &options);
    ...
    assert!(done.last_error.as_deref().unwrap_or("").contains("503"), "{:?}", done.last_error);
    assert!(!dest.exists(), "a failed transfer must not leave a file at the destination");
    assert!(sidecar_path(&dest).exists(), "the progress is kept so the download can be resumed");
}
...
fn a_client_error_on_a_segment_is_fatal_and_not_retried() {
    ...
    let transfer = Transfer::begin(DownloadSpec { dest, options, on_update: None }, probed, clearance).unwrap();
    let done = finish(&transfer);
    assert_eq!(done.state, TransferState::Failed);
...
fn a_file_that_changes_between_probe_and_transfer_fails_instead_of_being_spliced() {
    ...
    rig.server.update("/f.bin", |r| r.etag = Some("\"v2\"".to_string()));

    let transfer = Transfer::begin(DownloadSpec { dest: dest.clone(), options, on_update: None }, probed, clearance).unwrap();
    let done = finish(&transfer);

    assert_eq!(done.state, TransferState::Failed);
    assert!(done.last_error.as_deref().unwrap_or("").contains("changed"), "{:?}", done.last_error);
    assert!(!dest.exists());
}
...
fn a_server_whose_content_range_disagrees_with_the_request_is_rejected() {
    ...
    let options = Rig::options();
    let (probed, clearance) = rig.review("/f.bin", &dest, &options);
    rig.server.update("/f.bin", |r| r.content_range_override = Some(format!("bytes 7-7/{MIB}")));

    let done = finish(&Transfer::begin(DownloadSpec { dest, options, on_update: None }, probed, clearance).unwrap());
    assert_eq!(done.state, TransferState::Failed);
...
fn a_stalled_connection_is_revoked_and_its_segment_retried() {
    ...
    rig.server.serve("/f.bin", Resource::new(body(MIB)));
    let dest = rig.dest("f.bin");
    let options = DownloadOptions { stall_timeout: Duration::from_millis(300), ..Rig::options() };
    let (probed, clearance) = rig.review("/f.bin", &dest, &options);
    ...
    assert_eq!(done.state, TransferState::Completed, "{done:?}");
    assert!(started.elapsed() < Duration::from_secs(6), "the watchdog must not wait out the server's 8 s stall, took {:?}", started.elapsed());
    assert_eq!(std::fs::read(&dest).unwrap(), body(MIB));
...
fn start_and_pause(rig: &Rig, name: &str) -> (std::path::PathBuf, Snapshot) {
    rig.server.serve(&format!("/{name}"), slow_resource(4 * MIB, 10));
    let dest = rig.dest(name);
    let transfer = rig.begin(&format!("/{name}"), &dest, Rig::options());
    wait_until("some progress", || transfer.snapshot().completed_bytes > 300 * KIB as u64);
    transfer.pause();
...
fn pausing_saves_progress_and_a_new_begin_resumes_from_it() {
    ...
    assert!(first.completed_bytes >= paused.completed_bytes, "resuming must keep the saved progress: {} < {}", first.completed_bytes, paused.completed_bytes);
    let done = finish(&resumed);

    assert_eq!(done.state, TransferState::Completed);
    assert_eq!(std::fs::read(&dest).unwrap(), body(4 * MIB));
    // The second run fetched only what was missing.
    let fetched: u64 = ranged_requests(&rig.server).into_iter().skip(requests_before + 1).map(|(a, b)| b - a + 1).sum();
    assert!(fetched < 4 * MIB as u64, "resumed run refetched {fetched} bytes of a 4 MiB file");
}

#[test]
fn pausing_a_server_with_nothing_to_validate_against_discards_the_progress() {
    let rig = Rig::new();
    rig.server.serve("/f.bin", Resource { etag: None, last_modified: None, chunk: 16 * KIB, delay_per_chunk: Duration::from_millis(10), ..Resource::new(body(2 * MIB)) });
    let dest = rig.dest("f.bin");
    let transfer = rig.begin("/f.bin", &dest, Rig::options());
    assert!(!transfer.snapshot().resume_safe, "no ETag or Last-Modified: a pause can't be resumed safely");
    assert!(has_event(&transfer.snapshot(), "restart"), "the user is told up front: {:?}", transfer.snapshot().events);
    wait_until("some progress", || transfer.snapshot().completed_bytes > 100 * KIB as u64);

    transfer.pause();
    let paused = transfer.snapshot();
    assert_eq!(paused.state, TransferState::Paused);
    assert_eq!(paused.completed_bytes, 0, "progress is discarded, and the snapshot says so");
...
fn pause_and_cancel_on_a_finished_transfer_change_nothing() {
    ...
    assert_eq!(std::fs::read(&dest).unwrap(), body(10_000), "cancelling a completed download must not delete it");
}
...
fn dropping_a_running_transfer_leaves_it_resumable() {
    ...
    assert!(has_event(&resumed.snapshot(), "resumed"), "{:?}", resumed.snapshot().events);
...
fn a_remote_file_that_changed_since_the_pause_restarts_from_zero_with_the_new_content() {
    ...
    assert!(has_event(&first, "restarting from the beginning"), "{:?}", first.events);
    assert!(has_event(&first, "changed"), "the reason is given: {:?}", first.events);
    assert!(!has_event(&first, "resumed"));
    let done = finish(&transfer);
    assert_eq!(done.state, TransferState::Completed);
    assert_eq!(std::fs::read(&dest).unwrap(), replacement, "not one byte of the old file may survive in the result");
}
...
fn a_truncated_partial_file_restarts_from_zero() {
    let rig = Rig::new();
    let (dest, _) = start_and_pause(&rig, "big.bin");
    let part = std::fs::OpenOptions::new().write(true).open(part_path(&dest)).unwrap();
    part.set_len(1234).unwrap();
    drop(part);

    rig.server.update("/big.bin", |r| r.delay_per_chunk = Duration::ZERO);
    let transfer = rig.begin("/big.bin", &dest, Rig::options());
    assert!(has_event(&transfer.snapshot(), "restarting from the beginning"), "{:?}", transfer.snapshot().events);
    assert_eq!(finish(&transfer).state, TransferState::Completed);
    assert_eq!(std::fs::read(&dest).unwrap(), body(4 * MIB));
}
...
fn a_sidecar_for_a_different_url_is_not_resumed_from() {
    ...
    rig.server.serve("/other.bin", Resource::new(body(4 * MIB).iter().map(|b| b ^ 0xFF).collect()));
...
fn files_captured_at_an_arbitrary_moment_are_resumable_like_after_a_crash() {
    ...
    let dest = rig.dest("f.bin");
    let transfer = rig.begin("/f.bin", &dest, Rig::options());
    wait_until("a checkpoint with progress", || Sidecar::load(&dest).is_some_and(|s| s.segments.iter().any(|seg| seg.pos > seg.start + 100 * KIB as u64)));
    ...
    rig.server.update("/f.bin", |r| r.delay_per_chunk = Duration::ZERO);
    let resumed = rig.begin("/f.bin", &crashed, Rig::options());
    assert!(has_event(&resumed.snapshot(), "resumed"), "{:?}", resumed.snapshot().events);
    assert_eq!(finish(&resumed).state, TransferState::Completed);
    assert_eq!(std::fs::read(&crashed).unwrap(), body(4 * MIB), "a mid-flight checkpoint must resume to a correct file");
}
...
fn last_modified_alone_is_enough_to_pause_resume_and_to_detect_a_change() {
    let rig = Rig::new();
    rig.server.serve("/f.bin", Resource { etag: None, chunk: 16 * KIB, delay_per_chunk: Duration::from_millis(10), ..Resource::new(body(4 * MIB)) });
    let dest = rig.dest("f.bin");
    let transfer = rig.begin("/f.bin", &dest, Rig::options());
    assert!(transfer.snapshot().resume_safe, "Last-Modified is a usable validator");
    wait_until("some progress", || transfer.snapshot().completed_bytes > 300 * KIB as u64);
    transfer.pause();
    drop(transfer);
    ...
    rig.server.serve("/g.bin", Resource { etag: None, ..Resource::new(body(MIB)) });
    let options = Rig::options();
    let (probed, clearance) = rig.review("/g.bin", &other, &options);
    rig.server.update("/g.bin", |r| r.last_modified = Some("Thu, 22 Oct 2015 07:28:00 GMT".to_string()));
    let failed = finish(&Transfer::begin(DownloadSpec { dest: other, options, on_update: None }, probed, clearance).unwrap());
    assert_eq!(failed.state, TransferState::Failed);
    assert!(failed.last_error.as_deref().unwrap_or("").contains("changed"), "{:?}", failed.last_error);
}
...
fn a_server_that_hides_the_total_length_is_streamed_and_says_why() {
    ...
    rig.server.serve("/f.bin", Resource { content_range_override: Some("bytes 0-0/*".to_string()), ..Resource::new(body(200 * KIB)) });
    let dest = rig.dest("f.bin");
    let transfer = rig.begin("/f.bin", &dest, Rig::options());
    assert_eq!(transfer.snapshot().mode, TransferMode::SingleStream { reason: SingleStreamReason::UnknownLength });
    let done = finish(&transfer);
    assert_eq!(done.state, TransferState::Completed);
    assert_eq!(std::fs::read(&dest).unwrap(), body(200 * KIB));
}
...
fn segments_are_fetched_from_the_final_url_not_through_the_redirect_again() {
    let rig = Rig::new();
    rig.server.serve("/old", Resource { redirect_to: Some("/new".to_string()), ..Resource::new(Vec::new()) });
    ...
    let log = rig.server.requests();
    let probe_end = log.iter().position(|r| r.path == "/new").expect("the probe reached /new");
    assert!(log[probe_end + 1..].iter().all(|r| r.path == "/new"), "{log:?}");
}
...
fn a_plain_stream_cut_mid_body_starts_over_and_still_completes() {
    ...
    assert_eq!(std::fs::read(&dest).unwrap(), body(300 * KIB));
    assert!(done.retries >= 1);
    let transfer_requests = rig.server.requests().iter().skip(1).filter(|r| r.range.is_none()).count();
    assert_eq!(transfer_requests, 2, "the cut attempt and the full retry from byte 0");
}
...
fn the_event_log_is_bounded_and_drops_the_oldest_entries_first() {
    ...
    rig.server.serve("/f.bin", Resource::new(body(MIB)));
    let dest = rig.dest("f.bin");
    let options = DownloadOptions { max_retries: 100, retry_backoff_base: Duration::from_millis(1), retry_backoff_max: Duration::from_millis(2), ..Rig::options() };
    let (probed, clearance) = rig.review("/f.bin", &dest, &options);
    rig.server.update("/f.bin", |r| r.fail_statuses = vec![503; 60]);

    let done = finish(&Transfer::begin(DownloadSpec { dest, options, on_update: None }, probed, clearance).unwrap());
    assert_eq!(done.state, TransferState::Completed, "{done:?}");
    assert!(done.retries >= 40, "enough failures to overflow the log: {}", done.retries);
    assert_eq!(done.events.len(), 32, "the log keeps only the most recent 32 events");
    assert!(!has_event(&done, "byte ranges"), "the oldest event (the probe's finding) was dropped");
...
fn a_plain_stream_that_gets_a_server_error_is_retried() {
    let rig = Rig::new();
    rig.server.serve("/f.bin", Resource { honor_ranges: false, ..Resource::new(body(200 * KIB)) });
    let dest = rig.dest("f.bin");
    let options = Rig::options();
    let (probed, clearance) = rig.review("/f.bin", &dest, &options);
...
fn a_file_that_shrinks_under_an_unchanged_etag_is_caught_by_the_ranges_that_no_longer_fit() {
    ...
    rig.server.update("/f.bin", |r| r.body = Arc::new(body(MIB / 2)));
    let done = finish(&Transfer::begin(DownloadSpec { dest: dest.clone(), options, on_update: None }, probed, clearance).unwrap());
    assert_eq!(done.state, TransferState::Failed);
    assert!(done.last_error.as_deref().unwrap_or("").contains("changed"), "{:?}", done.last_error);
    assert!(!dest.exists());
...
fn a_range_answer_carrying_a_different_etag_or_last_modified_is_rejected() {
    ...
    // not be spliced in.
    for (what, change) in [
        ("ETag", (|r: &mut Resource| r.range_etag_override = Some("\"other\"".to_string())) as fn(&mut Resource)),
        ("Last-Modified", |r: &mut Resource| r.range_last_modified_override = Some("Fri, 01 Jan 2100 00:00:00 GMT".to_string())),
    ] {
        let rig = Rig::new();
        // Only Last-Modified is a validator for the second case.
        let etag = if what == "ETag" { Some("\"v1\"".to_string()) } else { None };
        rig.server.serve("/f.bin", Resource { etag, ..Resource::new(body(MIB)) });
        let dest = rig.dest("f.bin");
        let options = Rig::options();
        let (probed, clearance) = rig.review("/f.bin", &dest, &options);
        ...
        assert_eq!(done.retries, 0, "{what}: a version mismatch is not transient");
        assert!(done.last_error.as_deref().unwrap_or("").contains(what), "{what}: {:?}", done.last_error);
    }
}
...
fn a_failed_transfer_that_cannot_be_resumed_leaves_no_partial_files() {
    let rig = Rig::new();
    rig.server.serve("/f.bin", Resource { etag: None, last_modified: None, ..Resource::new(body(MIB)) });
    let dest = rig.dest("f.bin");
    let options = DownloadOptions { max_retries: 1, ..Rig::options() };
    let (probed, clearance) = rig.review("/f.bin", &dest, &options);
    rig.server.update("/f.bin", |r| r.fail_statuses = vec![503; 500]);
    let done = finish(&Transfer::begin(DownloadSpec { dest: dest.clone(), options, on_update: None }, probed, clearance).unwrap());
    assert_eq!(done.state, TransferState::Failed);
    assert!(!part_path(&dest).exists() && !sidecar_path(&dest).exists(), "with nothing to validate a resume against, keeping the bytes would only mislead");
}
...
fn an_existing_destination_is_refused_unless_overwriting_is_allowed() {
    ...
    assert!(matches!(refused, Err(DownloadError::DestinationExists(_))), "{refused:?}");
    assert_eq!(std::fs::read(&dest).unwrap(), b"precious", "a refused download must not touch the existing file");

    let allowed = rig.begin("/f.bin", &dest, DownloadOptions { overwrite: true, ..Rig::options() });
...
fn a_destination_that_appears_during_the_transfer_is_not_clobbered() {
    ...
    let done = finish(&transfer);
    assert_eq!(done.state, TransferState::Failed);
    assert!(done.last_error.as_deref().unwrap_or("").contains("already exists"), "{:?}", done.last_error);
...
fn a_clearance_cannot_be_spent_on_a_different_file_name() {
    ...
    let result = Transfer::begin(DownloadSpec { dest: rig.dest("sneaky.exe"), options, on_update: None }, probed, clearance);
    assert!(matches!(result, Err(DownloadError::ClearanceMismatch(_))), "{result:?}");
    assert!(!rig.dest("sneaky.exe").exists() && !part_path(&rig.dest("sneaky.exe")).exists());
}

#[test]
fn a_clearance_cannot_be_spent_on_a_different_url_size_or_type() {
    let rig = Rig::new();
    rig.server.serve("/f.bin", Resource::new(body(10_000)));
    rig.server.serve("/g.bin", Resource::new(body(10_000)));
    let dest = rig.dest("f.bin");
    let options = Rig::options();
    let (probed, clearance) = rig.review("/f.bin", &dest, &options);
    let (other_probe, _) = rig.review("/g.bin", &dest, &options);

    let cases: Vec<(&str, Probe)> = vec![
        ("url", Probe { url: other_probe.url.clone(), ..probed.clone() }),
        ("size", Probe { total: Some(10_001), ..probed.clone() }),
        ("content type", Probe { content_type: Some("text/html".to_string()), ..probed.clone() }),
    ];
    for (what, altered) in cases {
        let (_, fresh_clearance) = rig.review("/f.bin", &dest, &options);
        let result = Transfer::begin(DownloadSpec { dest: dest.clone(), options: options.clone(), on_update: None }, altered, fresh_clearance);
        assert!(matches!(result, Err(DownloadError::ClearanceMismatch(_))), "{what}: {result:?}");
    }
    drop(clearance);
}
...
fn a_snapshot_reports_speed_eta_segments_and_connections_while_running() {
    ...
    wait_until("speed, ETA and an active segment to show up", || {
        ...
        seen_active_segment |= s.segments.iter().any(|seg| seg.state == SegmentState::Active) && s.connections >= 1;
    ...
    assert!(has_event(&s, "byte ranges"), "the probe's finding is in the event log: {:?}", s.events);
    transfer.cancel();
}
...
fn the_update_callback_sees_progress_and_a_final_completed_snapshot() {
    ...
    let options = Rig::options();
    let (probed, clearance) = rig.review("/f.bin", &dest, &options);
    let spec = DownloadSpec { dest: dest.clone(), options, on_update: Some(Arc::new(move |snapshot: &Snapshot| sink.lock().unwrap().push(snapshot.clone()))) };

    let transfer = Transfer::begin(spec, probed, clearance).unwrap();
    finish(&transfer);

    let seen = seen.lock().unwrap();
    assert!(seen.len() >= 3, "expected several updates, got {}", seen.len());
    assert!(seen.iter().any(|s| s.state == TransferState::Active && s.completed_bytes > 0 && s.completed_bytes < MIB as u64), "an in-progress update");
...
fn wait_timeout_gives_up_when_the_transfer_has_not_settled() {
    ...
    transfer.cancel();
    assert_eq!(transfer.wait_timeout(Duration::from_secs(5)).unwrap().state, TransferState::Cancelled);
}
